...
Recently, State Space Models (SSMs) \cite{ssm,Mamba,VisionMamba}, exemplified by PointMamba \cite{PointMamba} and PCM \cite{pcm}, have gained attention due to their linear complexity. However, SSMs exhibit inherently limited spatial learning capability when applied to 3D point clouds. This limitation stems from two compounding factors. First, serializing 3D point clouds into 1D sequences inevitably displaces spatially adjacent points to distant sequence positions. Second, the SSM propagates information sequentially via the state matrix $\bar{A}$, where information between distant positions decays before reaching each other's states, preventing the model from effectively capturing local geometric structures. As illustrated on the left side of \cref{fig:analysis}, these two factors compound: spatially adjacent points become separated in the sequence, and the state decay further attenuates their mutual information flow, severely degrading the model's ability to learn spatial relationships.

Existing geometric feature extraction methods such as PointGA \cite{pointga} and PointMSGT \cite{pointmsgt} were designed for Transformers and CNNs, which possess stronger spatial learning capabilities. Transformers encode spatial proximity through attention weights derived from 3D coordinates, enabling direct interaction between any pair of points regardless of their sequence position. CNNs operate on local neighborhoods in the original 3D space through convolutional kernels, inherently preserving spatial structure. For these architectures, injecting geometric priors (e.g., normals and curvature) is effective because the model can readily leverage them through its spatial-aware mechanisms. However, SSMs lack such mechanisms---they can only propagate information along the serialized sequence order. Consequently, geometric priors injected into SSM inputs cannot be effectively utilized, as the underlying sequential propagation still fails to bridge spatially adjacent but sequentially distant points.
