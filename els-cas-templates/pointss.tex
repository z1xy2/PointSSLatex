...
\begin{figure*}[!t]
	...
	\caption{Qualitative comparison on S3DIS Area-5. (a) Large flat surface regions (e.g., walls, windows, columns, whiteboards): PTv3 produces fragmented predictions on spatially extended surfaces, while PointSS maintains consistent segmentation. (b) Complex boundary regions (e.g., bookshelf-clutter junctions, small objects): PTv3 produces blurred boundaries and misclassifications, while PointSS produces sharper and more accurate boundary predictions. Best viewed in color.}
	\label{fig:vis}
\end{figure*}

\section{Conclusion}

This paper presents PointSS, a geometry-aware multi-scale framework for point cloud analysis built upon State Space Models. GGAM and ASD-SSM address the two key limitations of existing SSM-based methods—serialization-induced spatial proximity loss and input-invariant state transition parameters—respectively. Experiments demonstrate strong performance across diverse benchmarks: 73.8\% mIoU on S3DIS Area 5 (outperforming PTv3 by 0.4\% and PCM by 4.0\%), 94.6\% OA on ModelNet40, and 80.9\% mIoU on nuScenes outdoor LiDAR segmentation (surpassing PTv3 at 80.4\% and Pamba at 80.4\%). PointSS achieves 9--19\% inference latency reduction over PTv3 across patch sizes 128--512, 59\% memory reduction at patch size 512 (6.0GB vs. 14.7GB), and stable scaling to patch size 2048 while PTv3 encounters OOM at 1024.

Despite these contributions, PointSS has several limitations. The scale constraint factors $\alpha_s$ are manually specified as fixed hyperparameters. While our systematic ablation study demonstrates robustness within reasonable ranges, automatic tuning mechanisms would further enhance generalizability across diverse datasets. In addition, the dual-serialization design in GGAM introduces additional computational overhead compared to single-stream methods.

Future work will explore automatically learning the scale constraint factors via learnable parameterization or neural architecture search to reduce the need for manual tuning. Extending PointSS to additional outdoor LiDAR benchmarks such as SemanticKITTI and Waymo Open Dataset, investigating more efficient geometric feature fusion strategies to reduce GGAM's computational overhead, and exploring PointSS's applicability to other 3D vision tasks such as object detection and instance segmentation are also directions we intend to pursue.
