...
However, these methods are primarily designed for Transformer or CNN architectures, where spatial proximity is inherently encoded in the model structure: Transformers maintain spatial awareness through attention weight patterns derived from 3D coordinates, while CNNs exploit local neighborhoods in the original 3D space via convolutional kernels. When applied to SSM-based methods, which convert point clouds into 1D sequences, neither attention mechanisms nor local receptive fields can be directly leveraged because of the limited spatial learning capability.
