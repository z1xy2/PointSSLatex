...
\begin{enumerate}
	...
	on S3DIS semantic segmentation, 94.6\% OA on ModelNet40 classification, and 80.9\% mIoU on nuScenes outdoor LiDAR segmentation,
...
\cref{tab:s3dis} summarizes the semantic segmentation performance on S3DIS Area 5. Without pre-training, PointSS achieves 73.8\% mean Intersection over Union (mIoU), outperforming all compared methods. Specifically, it surpasses the state-of-the-art Point Transformer V3 by 0.4\% and the SSM-based Pamba by 0.3\%. Furthermore, PointSS retains the linear time complexity of SSMs.

\begin{table*}
	\centering
	\caption{Performance Comparison of Semantic Segmentation on S3DIS. $^\dagger$Median of five independent runs, with standard deviations of $\pm$0.23\% (PCM, reproduced from the official implementation) and $\pm$0.19\% (PointSS). All other results are taken directly from the original papers.}
	\label{tab:s3dis}
	\begin{tabular}{@{}lll@{}}
		\toprule
		...
		PCM$^\dagger$\cite{pcm} & AAAI'25 & 69.8$^\dagger$ \\
...
\begin{table*}
	...
	\begin{tabular}{@{}lll@{}}
		...
		SphereFormer \cite{sphereformer} & CVPR'21 & 79.5 \\
...
As shown in \cref{tab:efficiency}, PointSS introduces additional parameters compared to PTv3 (52.2M vs. 46.2M, +13\%), primarily from the GGAM geometric feature extraction module. However, owing to the linear complexity of SSM-based state propagation, PointSS achieves substantially lower memory footprint: at patch size 512, PointSS consumes only 6.0GB compared to PTv3's 14.7GB, representing a 59\% reduction. More critically, while PTv3 encounters out-of-memory errors at patch size 1024 due to quadratic attention complexity, PointSS scales stably to patch size 2048, demonstrating that architectural design—rather than parameter count—determines practical scalability.
