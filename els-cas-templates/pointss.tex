...
\begin{enumerate}
	...
	on S3DIS semantic segmentation and 80.9\% mIoU on nuScenes outdoor LiDAR segmentation,
...
To evaluate the generalization capability of PointSS on outdoor LiDAR data, we conduct experiments on the nuScenes validation set. As shown in \cref{tab:nuscenes}, PointSS achieves 80.9\% mIoU, outperforming both Transformer-based methods (PTv3: 80.4\%, SphereFormer: 79.5\%) and SSM-based methods (Pamba: 80.4\%, PCM: not reported). This demonstrates that the geometry-aware multi-scale design of PointSS generalizes effectively from indoor scenes to outdoor autonomous driving scenarios, where point clouds exhibit different characteristics such as larger spatial extent and sparser density.

\begin{table*}
	\centering
	\caption{Semantic Segmentation Performance Comparison on nuScenes}
	\label{tab:nuscenes}
	\begin{tabular}{@{}lll@{}}
		\toprule
		\textbf{Method} & \textbf{Venue} & \textbf{mIoU (\%)} \\ \midrule
		...
		PointSS (Ours) & -- & \textbf{80.9} \\ \bottomrule
	\end{tabular}
\end{table*}
