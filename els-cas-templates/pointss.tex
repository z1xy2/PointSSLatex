...
To address these limitations, we propose PointSS, a geometry-aware multi-scale state space framework. As depicted in the middle of \cref{fig:analysis}, PointSS explicitly injects geometric priors through windowed graph construction to compensate for spatial proximity loss. Unlike PCM's fixed state transition parameters, PointSS generates them dynamically based on local geometric characteristics, enabling adaptive feature aggregation. PointSS further employs scale-decoupled parameterization: scale constraint factors remain fixed across scales to control hierarchical decay rates, while state transition parameters adapt to local geometry. This design avoids the parameter redundancy of PCM's per-scale independent parameterization. As shown on the right side of \cref{fig:analysis}, PointSS responds more effectively to local geometric variations, producing sharper, more accurate boundaries compared to other SSM methods.
...
PointSS addresses these limitations through explicit geometric prior injection, input-dependent state transitions, and scale-decoupled parameterization. While PointMamba and PCM rely solely on learned features from serialized sequences, PointSS constructs windowed local graphs to extract surface normals, curvature, and angular deviations, directly compensating for spatial proximity loss. This design choice is distinct from PointGA's trigonometric encoding \cite{pointga} and PointMSGT's triangular reconstruction \cite{pointmsgt}, which focus on geometric feature extraction but are not tailored to address the fundamental spatial proximity loss inherent in SSM-based serialization. Unlike PCM's fixed $\bar{A}$ parameters that apply identical state transitions across all spatial locations within each scale, PointSS generates $\bar{A}$ dynamically based on local geometric characteristics, enabling adaptive feature aggregation that responds to varying surface properties---processing flat surfaces and sharp boundaries with distinct transition dynamics. By decoupling scale control from spatial adaptation---where scale constraint factors remain fixed across scales to hierarchically control state decay rates, while $\bar{A}$ adapts to local geometry---PointSS avoids PCM's parameter redundancy from per-scale independent parameterization while maintaining effective multi-scale modeling. These design choices motivate the architecture of PointSS, which we describe in detail in the following section.
