...
Recently, State Space Models (SSMs) \cite{ssm,Mamba,VisionMamba}, have gained attention due to their linear complexity. However, SSMs exhibit inherently limited spatial learning capability when applied to 3D point clouds. This limitation stems from two compounding factors. As illustrated on the left side of \cref{fig:analysis}. First, serializing 3D point clouds into
1D sequences will inevitably sacrifice spatial proximity.some neighboring points can be far apart after serialization. Second, such a loss of
...
Existing geometric feature extraction methods such as PointGA \cite{pointga} and PointMSGT \cite{pointmsgt} were designed for Transformers and CNNs, which possess stronger spatial learning capabilities. Transformers encode spatial proximity through attention weights derived from 3D coordinates, enabling direct interaction between any pair of points regardless of their sequence position. CNNs operate on local neighborhoods in the original 3D space through convolutional kernels, inherently preserving spatial structure. For these architectures, injecting geometric priors is effective because the model can readily leverage them through its spatial-aware mechanisms. However, SSMs lack such mechanisms as mentioned above.

To address this, GGAM directly encodes neighborhood spatial context into features within serialization windows. Rather than relying on the SSM to learn spatial relationships from geometric priors, GGAM pre-computes and injects the spatial structure that serialization destroys. This allows PointSS to capture local geometric structures even at object boundaries (e.g., wall-pillar junctions), where spatially proximate points are far apart after serialization.
...
To address these limitations, we propose PointSS, a geometry-aware multi-scale state space framework. As depicted in the middle of \cref{fig:analysis}, PointSS directly encodes neighborhood spatial context into features within serialization windows via GGAM. GGAM addresses this by pre-computing and injecting the spatial structure that serialization destroys, rather than relying on the SSM to learn spatial relationships from geometric features. Leveraging these geometry-aware features, we introduce an adaptive multi-scale learning mechanism with scale-decoupled parameterization (ASD-SSM): fine scales maintain small state transition parameters for rapid decay to capture local geometric detail, while coarse scales maintain large values for slow decay to model long-range context. As shown on the right side of \cref{fig:analysis}, this design enables PointSS to respond more effectively to local geometric variations, producing sharper, more accurate boundaries compared to other SSM methods.
The main contributions of this work are as follows:

\begin{enumerate}
	\item We propose a Global Geometry-Aware Mechanism (GGAM) that directly
	encodes neighborhood spatial context into features through windowed
	graph construction and dual-serialization fusion, effectively mitigating
	the loss of spatial proximity and providing reliable geometric guidance
	for scale-adaptive learning.
...
However, these methods are primarily designed for Transformer or CNN architectures, where spatial proximity is inherently encoded in the model structure: Transformers maintain spatial awareness through attention weight patterns derived from 3D coordinates, while CNNs exploit local neighborhoods in the original 3D space via convolutional kernels. When applied to SSM-based methods, which convert point clouds into 1D sequences, neither attention mechanisms nor local receptive fields can be directly leveraged. The SSM's state $\mathbf{h}_t$ is an exponentially-weighted accumulation over the sequence history, where the state decay factor $\bar{A}$ controls information flow along the **sequence order**---not spatial proximity. Existing geometric feature extraction techniques such as PointGA and PointMSGT explicitly inject geometric priors (e.g., normals and curvature) but still rely on the SSM to learn spatial relationships from these features. However, the SSM was originally designed for sequential text data in natural language processing, where the sequence order inherently corresponds to semantic proximity---the i-th word naturally relates to the (i+1)-th word. Consequently, the SSM's state propagation mechanism cannot effectively learn spatial relationships from injected geometric features under serialization-induced proximity loss.
