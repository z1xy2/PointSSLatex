...
Recently, State Space Models (SSMs) \cite{ssm,Mamba,VisionMamba}, exemplified by PointMamba \cite{PointMamba} and PCM \cite{pcm}, have gained attention due to their linear complexity. However, SSMs exhibit inherently limited spatial learning capability when applied to 3D point clouds. This limitation stems from two compounding factors. First, As illustrated on the left side of \cref{fig:analysis}, some neighboring points can be far apart after serializing 3D point clouds into 1D sequences---points that are spatially close may become distant in the sequence. Second, the SSM propagates information sequentially via the state matrix $\bar{A}$, where state values decay exponentially with sequence distance. As illustrated on the left side of \cref{fig:analysis}, these two factors compound: spatially adjacent points become separated in the sequence, and the SSM's exponential state decay further attenuates their mutual information flow, severely degrading the model's ability to learn spatial relationships.

Existing geometric feature extraction methods such as PointGA \cite{pointga} and PointMSGT \cite{pointmsgt} were designed for Transformers and CNNs, which possess strong intrinsic spatial learning capabilities. Transformers encode spatial proximity through attention weights derived from 3D coordinates, enabling direct interaction between any pair of points regardless of their sequence position. CNNs operate on local neighborhoods in the original 3D space through convolutional kernels, inherently preserving spatial structure. For these architectures, injecting geometric priors (e.g., normals and curvature) is effective because the model can readily leverage them through its spatial-aware mechanisms. However, SSMs lack such mechanisms---they can only propagate information along the serialized sequence order. Consequently, geometric priors injected into SSM inputs cannot be effectively utilized, as the underlying sequential propagation still fails to bridge spatially adjacent but sequentially distant points.

To address this, GGAM directly encodes neighborhood spatial context into token features within serialization windows. Rather than relying on the SSM to learn spatial relationships from geometric priors, GGAM pre-computes and injects the spatial structure that serialization destroys. This allows PointSS to capture local geometric structures even at object boundaries (e.g., wall-pillar junctions), where spatially proximate points are far apart after serialization.
