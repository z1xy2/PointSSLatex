...
after serialization. While PointGA \cite{pointga} and PointMSGT \cite{pointmsgt} address geometric feature extraction through trigonometric encoding and triangular reconstruction respectively, they do not tackle the fundamental spatial proximity loss inherent in SSM-based serialization.
...
\begin{enumerate}
	...
	on S3DIS semantic segmentation and 81.0$\pm$0.4\% mIoU on nuScenes outdoor LiDAR segmentation,
...
details. Beyond general scene understanding, recent advances have extended point cloud analysis to domain-specific applications such as infrastructure inspection~\cite{bridgePointCloud}, demonstrating the broad applicability of deep learning methods across diverse point cloud tasks.
...
dependencies, limiting its ability to handle multi-scale representations. Beyond general scene understanding, point cloud analysis has been extended to diverse application domains. For instance, outdoor LiDAR-based semantic segmentation supports autonomous driving and intelligent transportation systems, where accurate 3D environment perception serves as the foundational component for higher-level tasks such as urban traffic flow prediction~\cite{ALI1,ALI2} and traffic pattern analysis~\cite{ALI3}.
...
architectures with hierarchical attention mechanisms. OctFormer~\cite{octformer2} extends octree structures for efficient multi-scale point cloud modeling. CCMNet~\cite{ccmnet} explores multi-scale and cross-type feature interactions for improved 3D learning. RandLA-Net~\cite{Randla} demonstrates that random sampling with a local feature aggregation module enables effective processing of large-scale scenes. MFINet~\cite{MFINet} further explores multi-scale feature interaction through hierarchical fusion strategies for point cloud registration, demonstrating the effectiveness of cross-scale feature aggregation. These approaches
...
\cref{tab:s3dis} summarizes the semantic segmentation performance on S3DIS Area 5. Without pre-training, PointSS achieves 73.8\% mean Intersection over Union (mIoU), achieving competitive performance among compared methods. Specifically, it achieves statistically significant improvements over Point Transformer V3 (+0.4\%, p=0.012) and Pamba (+0.3\%, p=0.035). Furthermore, PointSS retains the linear time complexity of SSMs.

\textbf{Statistical Validation.} To assess the reliability of our results, we conduct eight independent training runs with different random seeds, yielding 73.2\%, 73.4\%, 73.6\%, 73.8\%, 73.8\%, 74.0\%, 74.1\%, and 74.3\% (mean=$73.78\%\pm0.37\%$). One-sample t-tests comparing the PointSS mean against baselines yield: $t(7)=2.90$, $p=0.012$ vs. PTv3 (73.4\%); $t(7)=2.14$, $p=0.035$ vs. Pamba (73.5\%). While the margins are modest (+0.38\% over PTv3, +0.28\% over Pamba), PointSS delivers this competitive performance with substantially lower computational cost (Section~4.4).

\begin{table*}
	\centering
	\caption{Performance Comparison of Semantic Segmentation on S3DIS. $^\dagger$Mean of eight independent runs. PointSS: 73.2\%, 73.4\%, 73.6\%, 73.8\%, 73.8\%, 74.0\%, 74.1\%, 74.3\%. One-sample t-tests: $t(7)=2.90$, $p=0.012$ vs. PTv3 (73.4\%); $t(7)=2.14$, $p=0.035$ vs. Pamba (73.5\%). PCM reproduced from official implementation. All other results from original papers.}
	\label{tab:s3dis}
	\begin{tabular}{@{}lll@{}}
		\toprule
		...
		PCM$^\dagger$\cite{pcm} & AAAI'25 & 70.1$^\dagger$ \\
		Pamba\cite{pamba} & AAAI'25 & 73.5 \\
		HybridTM\cite{hybridtm} & IROS'25 & 72.1 \\
		PointSS$^\dagger$ (Ours) & -- & \textbf{73.8} \\ \bottomrule
	\end{tabular}
\end{table*}
...
\begin{table*}
	...
	\begin{tabular}{@{}lll@{}}
		...
		Point Transformer V3 \cite{ptv3} & CVPR'24 & 80.4 \\ \midrule
		\multicolumn{3}{l}{\textit{w/ Mamba}} \\
...
\subsection{Computational Cost and Performance Analysis}

\begin{table}[t]
\centering
\caption{Computational cost and performance comparison on S3DIS Area 5. Peak memory measured at patch size 512 with batch size 6. Inference time normalized relative to PTv3 baseline. PTv3 encounters out-of-memory errors at patch size 1024.}
...
Compared to PCM, another SSM-based method, PointSS achieves 3.7\% higher mIoU (73.8\% vs. 70.1\%) with only a modest increase in memory footprint (6.0GB vs. 5.2GB at patch size 512) and comparable inference speed (0.81--0.91$\times$ vs. 0.75--0.84$\times$ relative to PTv3). Both methods demonstrate stable scaling to patch size 2048, confirming the scalability advantage of SSM-based architectures. However, PointSS achieves substantially better accuracy while maintaining competitive efficiency.
...
Experimental results indicate that uniform $\alpha_s$ configurations consistently underperform relative to non-uniform configurations, confirming that distinct scales require scale-specific state-decay characteristics. The best performance is achieved when $\alpha_s$ increases progressively from fine to coarse scales (0.3, 0.6, 0.9), validating the design philosophy: fine scales benefit from rapid decay to capture local geometric detail, while coarse scales require slow decay to maintain long-range state memory. Importantly, the method demonstrates robustness within reasonable ranges. Configurations following the monotonically increasing pattern achieve stable performance with minimal variation (73.3\%--73.8\%, $<$0.5\% difference), and moderate boundary adjustments are well-tolerated (varying $\alpha_1 \in [0.2, 0.4]$ yields 73.2\%--73.3\%). Only configurations that violate the underlying physical principle (e.g., reversed ordering 0.9$\rightarrow$0.3, dropping to 66.5\%) cause significant degradation. This indicates that the scale constraint factors encode a robust design principle that generalizes across reasonable hyperparameter choices without requiring extensive dataset-specific tuning.
...
The performance gains of PointSS arise from addressing two structural deficiencies
in existing SSM-based methods: serialization-induced spatial proximity loss and
input-invariant state transition parameters.

\textbf{Why GGAM recovers spatial proximity.}
Serialization systematically displaces spatially adjacent points into distant sequence
positions, causing their interactions to decay before reaching each other's hidden states.
GGAM is effective because its windowed graph construction is \emph{aligned} with the
serialization granularity---it explicitly reconstructs the local neighborhoods that
serialization disrupts, injecting geometric priors precisely where proximity loss is most
severe. This is confirmed by the stratified gains in \cref{tab:ggam_fragmentation}: points
in the highest proximity-loss group (20--30\%) gain +5.4\% IoU, while low-loss points gain
only +0.7\%. Category-wise, the largest gains concentrate in geometrically complex regions
(window +3.5\%, clutter +3.3\%, door +3.0\%), where boundary accuracy depends critically
on local neighborhood integrity---regions that globally applied methods such as
PointGA~\cite{pointga} and PointMSGT~\cite{pointmsgt} cannot systematically address.

\textbf{Why dynamic $\bar{A}$ parameterization outperforms static alternatives.}
Existing SSMs apply a shared, input-invariant $\bar{A}$ uniformly across all scales and
spatial locations, implicitly treating flat surfaces and sharp boundaries identically.
ASD-SSM resolves this by decoupling the decay \emph{range} from its \emph{content
	adaptation}: the scale constraint factor $\alpha_s$ fixes the value range per scale (fine
$\bar{A}_1 \in [0,0.3]$ for rapid local response; coarse $\bar{A}_3 \in [0,0.9]$ for
long-range memory), while the patch-level generator adapts $\bar{A}$ dynamically within
that range based on local geometry. Two ablation results confirm this: Independent
Parameters (71.6\%) already outperforms Shared Parameters (70.8\%), yet ASD-SSM (73.8\%)
substantially exceeds both, showing that \emph{dynamic, input-dependent} generation is the
decisive factor. Reversing the $\alpha_s$ ordering to (0.9, 0.6, 0.3) causes a drop to
66.5\%, confirming that the monotonically increasing decay pattern encodes a physically
motivated design principle rather than a tuned hyperparameter.
