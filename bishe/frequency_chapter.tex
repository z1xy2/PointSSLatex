...
对于包含多个场景的批次数据，我们为每个场景独立构建图，避免跨场景的信息泄漏。由于序列化顺序已预先计算，无需额外排序开销。
...
消融实验验证了分量独立建模相比参数共享策略的优势，Chebyshev多尺度SSM在S3DIS数据集上达到70.8\% mIoU（相比Baseline提升0.5\%）。该模块与空域方法的互补性将在第六章实验部分进行整体验证。
