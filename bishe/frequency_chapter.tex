...
Chebyshev多尺度SSM的整体流程包括四个核心模块：（1）序列化窗口图构建，利用空间填充曲线的局部性高效构建邻域图；（2）Chebyshev多项式多尺度展开，将点云特征展开为多个空间尺度的分量；（3）分量独立建模，使用独立的Mamba处理各尺度分量；（4）多尺度融合，通过可学习权重自适应融合多尺度特征。接下来将依次介绍各模块的设计细节。
...
\end{equation}
其中$\sigma$为尺度参数，设为所有边长度的中位数。该权重设计使得空间距离近的点具有更强的连接权重，距离远的点影响快速衰减，为后续多尺度分解提供合理的邻域拓扑结构。
...
对于第$\ell$个分量的特征$X^{(\ell)} = T_\ell(\tilde{L})X \in \mathbb{R}^{N \times C}$，我们使用Point Transformer V3的序列化顺序（已预先计算）将其重排为序列：
\begin{equation}
X^{(\ell)}_{seq} = X^{(\ell)}[\pi], \quad \pi = \text{serialized\_order}
\end{equation}
其中$\pi$为序列化索引（如Hilbert曲线或Z-order曲线的排序）。注意，所有分量使用\textbf{相同的序列化顺序}$\pi$，这保证了分量间的空间对应关系。

第$\ell$个分量的Mamba模块定义为：
\begin{equation}
Y^{(\ell)} = \text{Mamba}_\ell(X^{(\ell)}_{seq})
\end{equation}
其中$\text{Mamba}_\ell$为第$\ell$个分量专属的Mamba块，包含双向SSM（前向+后向）和残差连接。在参数设计上，各分量Mamba的参数完全独立，$\text{Mamba}_\ell$与$\text{Mamba}_j$（$\ell \neq j$）不共享权重。为保持一致性，所有Mamba使用相同的状态维度$d_{state}$（实验中设为16）。同时，每个分量输出后应用LayerNorm，稳定训练过程。

形式化地，分量$\ell$的完整处理流程为：
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
其中$\text{Concat}(X, Y_{multi}) \in \mathbb{R}^{N \times 2C}$将原始特征和多尺度特征拼接，$\text{MLP}$为2层感知机（隐藏层使用GELU激活），输出维度为$C$。最后通过残差连接得到增强后的特征$X' \in \mathbb{R}^{N \times C}$。

该设计有两个优势：（1）拼接操作保留了原始特征信息，避免多项式变换导致的信息损失；（2）残差连接使模型能够自适应地选择多尺度增强的强度，在不同stage和数据集上具有更好的适应性。

与ASD-SSM类似，Chebyshev多尺度SSM也采用层级自适应配置。在网络的不同stage，多尺度建模的需求不同。对于浅层stage（1-2），特征的空间分辨率高，点云的局部几何信息丰富，启用Chebyshev多尺度SSM带来显著增益。而对于深层stage（3-5），特征已高度抽象，多尺度分解的边际收益递减，可选择性关闭以节省计算。

实验中，我们在前3个stage启用Chebyshev多尺度SSM，后2个stage使用标准Mamba，取得了性能与效率的最佳平衡。

\section{消融实验}

通过系统消融实验，分析Chebyshev多尺度SSM中关键设计选择对模型性能的影响，重点考察多项式阶数、Mamba独立性、融合策略以及图构建方法等因素。所有实验均在S3DIS数据集上进行。
...
实验结果表明，$K=1$（仅原始特征+1跳邻域）性能最低，说明单一空间尺度不足以捕获复杂的几何结构。随着阶数增加，模型性能显著提升，验证了多尺度分解的必要性。$K=3$时达到最优性能，此时包含0跳、1跳、2跳三个空间尺度的分量，足以覆盖点云的主要几何结构层次。继续增加到$K=4,5$时性能趋于饱和，但参数量和计算开销显著增加。这是因为更大感受野对应的结构信息在点云局部邻域中边际收益有限。综合考虑，$K=3$为最佳选择。
...
参数共享策略 & 参数量 & mIoU (\%) \\
\midrule
所有分量共享Mamba & 1.0$\times$ & 73.8 \\
低阶/高阶各自共享（2组） & 1.08$\times$ & 74.5 \\
\textbf{每个分量独立Mamba} & \textbf{1.15$\times$} & \textbf{75.2} \\
\bottomrule
\end{tabular}
\end{table}

实验显示，所有分量共享单一Mamba时性能受限，虽然参数量最少但难以同时适应小感受野分量的局部特性和大感受野分量的全局特性。将低阶分量（$\ell=0,1$）和高阶分量（$\ell=2$）分为两组，各组内共享Mamba，性能有所提升，验证了分组建模的价值。每个分量使用独立Mamba时达到最优性能，相比分组方案进一步改善。虽然参数量增加15\%，但增益显著，证明尺度特定建模的必要性。

\subsection{多尺度融合策略}

表\ref{tab:fusion_strategy}对比了不同多尺度融合方法的性能。
...
均匀加权作为最简单的基线，说明即使不学习权重，多尺度融合也能带来增益。使用线性层对拼接的多尺度特征进行投影，性能进一步改善。本文采用的Softmax加权融合达到最优性能，表明不同空间尺度分量的重要性确实存在差异，可学习权重能够自适应地调整各分量的贡献。我们还尝试了更复杂的注意力机制融合（用查询-键-值attention聚合各尺度特征），但性能反而略有下降。这可能是因为过度参数化导致过拟合，简单的Softmax加权已足够有效。


\subsection{与空域方法的对比}

表\ref{tab:comparison_spatial}对比了Chebyshev多尺度SSM与空域方法的性能。
...
+ GGAM & 空域 & 73.2 \\
+ ASD-SSM & 空域 & 73.5 \\
+ Chebyshev多尺度SSM & 图多项式 & 75.2 \\
\midrule
\textbf{+ GGAM + ASD-SSM + Chebyshev多尺度SSM} & \textbf{空域+图多项式} & \textbf{76.8} \\
\bottomrule
\end{tabular}
\end{table}

实验表明，单独使用Chebyshev多尺度SSM相比Baseline提升明显，验证了基于图多项式的多尺度建模的有效性。更重要的是，将Chebyshev多尺度SSM与空域方法（GGAM + ASD-SSM）结合后，性能达到76.8\%，展现出两类方法的互补性：空域方法擅长捕获局部几何细节和序列化邻域结构，Chebyshev多尺度SSM通过图拉普拉斯递推聚合不同跳数的邻域信息，两者结合能够更全面地刻画点云的多尺度几何特征。
...
实验结果表明，Chebyshev多尺度SSM在S3DIS数据集上达到75.2\% mIoU（相比Baseline提升3.3\%），消融实验验证了分量独立建模的必要性。与空域方法（GGAM + ASD-SSM）结合后，完整的PointSS系统在S3DIS上达到76.8\% mIoU，验证了图多项式建模与空域建模的互补性。
