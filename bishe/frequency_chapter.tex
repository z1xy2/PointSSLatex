...
Chebyshev多尺度SSM的整体流程包括四个核心模块，如\cref{label}：（1）序列化窗口图构建，利用空间填充曲线的局部性高效构建邻域图；（2）Chebyshev多项式多尺度展开，将点云特征展开为多个空间尺度的分量；（3）分量独立建模，使用独立的Mamba处理各尺度分量；（4）多尺度融合，通过可学习权重自适应融合多尺度特征。接下来将依次介绍各模块的设计细节。
