...
此外，我们对比了不同的多尺度融合方式，包括均匀加权（74.6\%）、线性层投影（74.9\%）、注意力机制（75.1\%）和Softmax加权（75.2\%）。结果表明Softmax加权融合已足够有效，更复杂的注意力机制反而因过度参数化略有下降。
...
实验表明，单独使用Chebyshev多尺度SSM相比Baseline提升明显，验证了基于图多项式的多尺度建模的有效性。更重要的是，将Chebyshev多尺度SSM与空域方法（GGAM + ASD-SSM）结合后，性能达到75.9\%，展现出两类方法的互补性：空域方法擅长捕获局部几何细节和序列化邻域结构，Chebyshev多尺度SSM通过图拉普拉斯递推聚合不同跳数的邻域信息，两者结合能够更全面地刻画点云的多尺度几何特征。
...
实验结果表明，Chebyshev多尺度SSM在S3DIS数据集上达到75.2\% mIoU（相比Baseline提升3.3\%），消融实验验证了分量独立建模的必要性。与空域方法（GGAM + ASD-SSM）结合后，完整的PointSS系统在S3DIS上达到75.9\% mIoU，验证了图多项式建模与空域建模的互补性。
