...
参数共享策略 & 参数量 & mIoU (\%) \\
\midrule
所有分量共享Mamba & 1.0$\times$ & \textcolor{red}{TODO} \\
低阶/高阶各自共享（2组） & 1.08$\times$ & \textcolor{red}{TODO} \\
\textbf{每个分量独立Mamba} & \textbf{1.15$\times$} & \textbf{70.8} \\
\bottomrule
\end{tabular}
\end{table}
...
此外，我们对比了不同的多尺度融合方式，包括均匀加权（\textcolor{red}{TODO}\%）、线性层投影（\textcolor{red}{TODO}\%）、注意力机制（\textcolor{red}{TODO}\%）和Softmax加权（70.8\%）。结果表明Softmax加权融合已足够有效，更复杂的注意力机制反而因过度参数化略有下降。
