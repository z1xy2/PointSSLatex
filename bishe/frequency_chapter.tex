...
对于第$\ell$个分量的特征$X^{(\ell)} = T_\ell(\tilde{L})X \in \mathbb{R}^{N \times C}$，直接沿用前文所述的序列化顺序$\pi$，按该顺序送入对应的Mamba模块进行序列建模：
\begin{equation}
X^{(\ell)}_{seq} = X^{(\ell)}[\pi]
\end{equation}
其中$\pi$为Point Transformer V3已预先计算的序列化索引（Hilbert曲线或Z-order曲线排序）。所有分量使用\textbf{相同的序列化顺序}$\pi$，保证了分量间的空间对应关系。
