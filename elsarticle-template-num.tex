...
\end{equation}
与传统的KNN方法相比，窗口化邻域构建方法采用\textbf{效率优先}的设计理念：牺牲局部邻域搜索精度换取显著的计算效率提升（复杂度从$O(N\log N)$降至$O(N)$，20万点规模下实现70倍加速），将节省的计算资源用于扩展模型规模。这一权衡策略受Point Transformer V3\cite{ptv3}启发，PTv3证明了通过窗口化邻域节省的资源可用于扩大感受野（16点$\rightarrow$1024点）和增加参数量，最终在多个基准上超越KNN方法。

窗口化邻域的\textbf{精度代价}：如\cref{tab:split_and_repair}所示，33\%的点存在K近邻割裂（序列距离>500），严重时性能损失达22.0个百分点。我们采用\textbf{双重补偿}：（1）双序列化互补（Z-order+Hilbert）降低割裂风险；（2）GGAM全局修复：窗口化邻域提供粗略几何先验，GGAM通过交叉注意力在全局范围内修复割裂点的几何信息（详见\cref{tab:split_and_repair}）。
