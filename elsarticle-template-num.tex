...
\end{equation}
与传统的KNN方法相比，窗口化邻域构建方法具有以下关键优势：（1）\textbf{计算效率}：避免昂贵的距离计算和排序操作，在20万点规模下实现70倍加速；（2）\textbf{可扩展性}：KNN方法的高计算复杂度限制了大规模点云与大模型的应用，而窗口化方法为点云大模型发展提供了可行方案；（3）\textbf{空间局部性}：Z-order和Hilbert编码具有良好的空间局部性保持特性；（4）\textbf{多视角互补}：双序列化策略使同一点在两种编码下获得不同邻域视角，有效弥补单一序列化的局限。

在窗口化邻域内，我们通过PCA方法估计几何特征（法向量和曲率）。虽然相比真实KNN邻域，窗口化邻域在边界和高曲率区域存在一定近似误差，但在合理的邻域尺度下，PCA仍能有效捕捉局部主导的几何结构\cite{PointNet++}。更重要的是，窗口化邻域计算的几何特征主要用于提供初始几何先验，而GGAM模块通过双序列化交叉注意力机制突破Mamba的线性状态传递限制，直接在两个序列空间中关联割裂点的几何信息，两者协同工作实现准确的点云分析。
