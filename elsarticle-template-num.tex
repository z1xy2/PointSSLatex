...
分析表明，平均割裂占比与GGAM提升幅度总体呈现正相关趋势。大平面类别（地板、墙、天花板）的平均割裂占比较低（5-8\%），GGAM提升有限（+1.2-1.4\%），因为这些类别本身序列化效果较好，几何结构简单且连续性强。边界密集类别（门、柱子、窗户）的平均割裂占比显著升高（17-19\%），GGAM带来的性能提升也更为显著（+5.3-5.8\%），证明显式几何先验对复杂边界的有效补偿能力。值得注意的是，柱子虽然割裂占比（18\%）略低于窗户（19\%），但由于其细长结构的几何敏感性，GGAM带来了更高的提升（+5.8\%）。中等复杂度的家具类别（桌子、书架、沙发、椅子）展现出中等程度的割裂占比（11-15\%）和相应的性能提升（+3.1-4.3\%）。特别值得注意的是，椅子和沙发尽管具有相同的平均割裂占比（15\%），但椅子的复杂结构使其获得了略高的提升（+4.3\% vs +4.2\%）。对于几何最复杂的杂物类别，其平均割裂占比高达22\%，而GGAM仍能带来+6.5\%的显著提升，充分说明显式几何先验能够有效增强模型对细节几何信息的学习。这一系统性的改善模式验证了GGAM的设计初衷：对于几何结构敏感且易被序列化割裂的类别，显式几何先验注入能够提供更显著的性能改善，而对于几何结构简单、割裂程度较低的类别，则保持稳定的基础性能。
