...
点云作为三维场景的直接表征形式，被广泛应用于自动驾驶、机器人导航以及增强现实（AR）/虚拟现实（VR）等领域。与二维图像相比，点云具有稀疏性、无序性与非规则采样等特点，这使得其特征提取与结构建模面临更高难度。近年来，激光雷达与深度相机等传感器成本下降、采集精度提升，大规模高密度点云数据的获取愈发便捷，推动了点云分析在复杂场景中的落地需求。然而，点数规模的增长与场景结构的复杂化，也对模型的计算效率、全局上下文建模能力以及几何细节刻画能力提出了更高要求。
