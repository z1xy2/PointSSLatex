...
分析表明，GGAM的提升幅度与类别的平均割裂占比呈正相关。大平面类别（地板、墙、天花板）割裂占比较低（5-8\%），几何结构简单，GGAM提升有限（+1.2-1.4\%）。中等复杂度的家具类别（桌子、书架、椅子、沙发）割裂占比为11-15\%，相应提升为+3.1-4.3\%。而边界密集类别（门、柱子、窗户）和复杂杂物的割裂占比升高（17-22\%），GGAM带来了更显著的性能改善（+5.3-6.5\%），验证了显式几何先验对复杂几何结构的补偿能力。
...
组件贡献分析显示各模块呈现强协同效应。几何特征作为基础组件贡献+0.8\% mIoU。双序列化策略单独加入时性能轻微下降0.1\%，但配合交叉注意力后带来跳跃性提升，验证了双序列化与交叉注意力的强协同作用。门控融合和几何一致性损失各额外贡献+0.3\% mIoU。完整的GGAM相比基线提升2.2\% mIoU，消融路径揭示了模块间的依赖关系：几何特征提供基础，双序列化需要交叉注意力才能发挥作用，门控融合和几何损失在此基础上进一步优化。
...
从\cref{tab:paramgen_design}可以看出，全局特征和尺度嵌入均有贡献，两者结合的提升（+1.8\%）超过单独使用之和（+1.6\%），展现出协同效应。调制强度$\lambda=0.1$时达到最优平衡，过小导致尺度差异化不足，过大则影响训练稳定性。
