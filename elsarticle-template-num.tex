...
\textbf{尺度感知参数生成器：}ASD-SSM通过尺度感知参数生成器（ParamGen）为不同尺度生成定制化的参数偏移量。对于尺度$s$，点云特征已按Patch划分为$\mathbf{P}_s \in \mathbb{R}^{M \times P \times C}$，其中$M = \lceil N/P \rceil$为Patch数量。值得注意的是，参数生成过程对每个Patch独立进行，即以Patch为维度不同的SSM参数偏移量。
...
\end{equation}
其中$\|$表示拼接操作。基于条件向量$\mathbf{c}_s$，参数生成器通过三层前馈网络生成该Patch的SSM参数偏移量：
\begin{equation}
	\Delta\overline{A}_s^{raw}, \Delta\overline{B}_s^{raw}, \Delta\overline{C}_s^{raw} = \text{ParamGen}(\mathbf{c}_s) \in \mathbb{R}^{D}
\end{equation}
其中$D$为SSM的状态空间维度（在我们的实现中$D=C$）。需要强调的是，上述参数生成过程对$M$个Patch独立并行执行，每个Patch都拥有其定制化的参数偏移量。这种设计使得模型能够根据不同Patch的局部几何特性自适应调整状态转移行为，在全局共享SSM架构的同时实现局部内容感知的参数调制。
...
\textbf{参数偏移量的应用：}生成的参数偏移量$\Delta\overline{A}_s, \Delta\overline{B}_s, \Delta\overline{C}_s \in \mathbb{R}^{D}$通过行广播（row-wise broadcasting）机制作用于基础SSM参数矩阵。对于第$s$个尺度的单个Patch，其有效参数为：
\begin{equation}
	\begin{aligned}
		\overline{A}_s &= \overline{A}_{base} \oplus \lambda_A \cdot \Delta\overline{A}_s \\
		\overline{B}_s &= \overline{B}_{base} \oplus \lambda_B \cdot \Delta\overline{B}_s \\
		\overline{C}_s &= \overline{C}_{base} \oplus \lambda_C \cdot \Delta\overline{C}_s
	\end{aligned}
	\label{eq:param_modulation}
\end{equation}
其中$\oplus$表示按行广播加法（row-wise broadcast addition），即矩阵$\overline{A}_{base} \in \mathbb{R}^{D \times D}$的每一行均加上偏移向量$\lambda_A \cdot \Delta\overline{A}_s \in \mathbb{R}^{D}$。$\overline{A}_{base}, \overline{B}_{base}, \overline{C}_{base} \in \mathbb{R}^{D \times D}$为所有尺度和Patch共享的基础SSM参数矩阵，$\lambda_A$、$\lambda_B$、$\lambda_C$为调制强度系数（实验中设为0.1）。这种设计在保持参数效率（偏移量参数量为$O(D)$）的同时，允许偏移向量同时调制参数矩阵的所有行，从而更灵活地适应不同尺度的状态转移需求。由于每个Patch内的SSM操作相互独立，为简化表示，后续公式中省略Patch索引。
...
\textbf{状态空间更新：}对于第$s$个尺度的每个Patch内的点云特征序列，其状态空间更新遵循标准SSM形式：
\begin{equation}
	\begin{aligned}
...
\end{equation}
其中$\mathbf{h}_k^{(s)} \in \mathbb{R}^{D}$是尺度$s$的隐状态向量，$\mathbf{u}_k^{(s)} \in \mathbb{R}^{D}$是Patch内第$k$个点的输入向量（$k=1,2,\ldots,P$），$\overline{D}_{base} \in \mathbb{R}^{D \times D}$为共享的直通连接参数矩阵。由于$\overline{A}_s, \overline{B}_s, \overline{C}_s, \overline{D}_{base}$均为对角矩阵，矩阵-向量乘法可高效实现为逐元素乘法，计算复杂度为$O(D)$而非一般矩阵乘法的$O(D^2)$。
...
从\cref{tab:paramgen_design}可以看出，仅使用尺度嵌入时性能提升0.7\% mIoU，证明了为不同尺度赋予独特标识的重要性。仅使用全局特征时提升0.9\% mIoU，说明内容自适应的参数调制更为关键。同时使用两者的提升（+1.8\%）大于单独使用之和（+0.7\% + +0.9\% = +1.6\%），展现出明显的协同效应。调制强度方面，$\lambda=0.1$时达到最优平衡，过小（0.05）导致参数偏移不足，过大（0.2, 0.3）则引入过多扰动，影响训练稳定性。
