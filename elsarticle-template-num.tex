...
\end{equation}
其中$\overline{A}_{base} \in \mathbb{R}^{D}$为所有尺度和Patch共享的基础状态转移参数向量（对角矩阵的对角元素），$\lambda_A$为调制强度系数（实验中设为0.1）。由于每个Patch内的SSM操作相互独立，为简化表示，后续公式中省略Patch索引。
...
\begin{equation}
	\begin{aligned}
		...
		\mathbf{y}_k^{(s)} &= \overline{C}_s \mathbf{h}_k^{(s)} + \overline{D} \mathbf{u}_k^{(s)}
	\end{aligned}
	\label{eq:asd_ssm_update}
\end{equation}
其中$\mathbf{h}_k^{(s)}$是尺度$s$的隐状态向量，$\mathbf{u}_k^{(s)}$是Patch内第$k$个点的输入向量（$k=1,2,\ldots,P$），$\overline{D}$为直通连接参数。参数$\overline{A}_s, \overline{B}_s, \overline{C}_s$的具体维度和离散化细节遵循Mamba论文\cite{Mamba}的标准实现。值得强调的是，ASD-SSM的创新点在于$\overline{A}_s$的尺度自适应生成，而不是对SSM基本形式的改变。
