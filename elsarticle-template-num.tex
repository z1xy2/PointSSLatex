...
\end{equation}
与传统的KNN方法相比，窗口化邻域构建方法采用效率优先的设计理念：牺牲局部邻域搜索精度换取显著的计算效率提升（复杂度从$O(N\log N)$降至$O(N)$，20万点规模下实现70倍加速），将节省的计算资源用于扩展模型规模。这一权衡策略受Point Transformer V3\cite{ptv3}启发，PTv3证明了通过窗口化邻域节省的资源可用于扩大感受野（16点$\rightarrow$1024点）和增加参数量，最终在多个基准上超越KNN方法。

窗口化邻域的\textbf{精度代价与补偿机制}：如\cref{tab:split_and_repair}所示，33\%的点存在K近邻割裂（序列距离>500）。对于这些割裂点，窗口化邻域虽然不能提供精确的几何特征，但仍能提供初步的几何信息作为粗略先验；而对于其余67\%的点，窗口化邻域能够提供相对准确的几何特征。我们采用\textbf{双重补偿}策略：（1）双序列化互补（Z-order+Hilbert）降低割裂风险；（2）GGAM全局修复：利用67\%准确点的几何信息，通过交叉注意力在全局范围内修复割裂点的几何特征，将严重割裂导致的22.0个百分点性能损失降低至可接受范围（详见\cref{tab:split_and_repair}）。
...
实验结果表明，在Hilbert曲线序列化下，33\%的点（13.3万/40.3万）存在不同程度的割裂问题（割裂近邻占比>10\%）。割裂程度与分割性能呈显著负相关，当一个点的割裂近邻占比从0-10\%增加到30-50\%时，其分割IoU从74.0\%降至52.0\%，性能损失达22.0个百分点，证明割裂问题显著影响模型对几何结构的理解能力。GGAM通过全局几何感知机制有效缓解了这一问题，割裂程度越严重，GGAM的绝对提升越显著，从0.8\%增加到8.0\%，证明全局几何感知能够有效跨越序列距离建立几何关联。整体而言，GGAM使所有点的平均IoU从70.3\%提升至72.5\%，绝对提升2.2\%，验证了全局几何感知机制的有效性。
