...
\textbf{尺度感知参数生成器：}ASD-SSM的核心创新在于为不同尺度自适应生成差异化的状态转移参数$\overline{A}_s$。对于尺度$s$，点云特征已按Patch划分为$\mathbf{P}_s \in \mathbb{R}^{M \times P \times C}$，其中$M = \lceil N/P \rceil$为Patch数量。参数生成过程对每个Patch独立进行，每个Patch生成其专属的$\Delta\overline{A}_s$（状态转移偏移量）。
...
\end{equation}
其中$\|$表示拼接操作。基于条件向量$\mathbf{c}_s$，参数生成器通过轻量级前馈网络生成该Patch的状态转移偏移量$\Delta\overline{A}_s^{raw} \in \mathbb{R}^{D}$（$D$为SSM的状态空间维度）。对于选择性参数$\overline{B}_s$和$\overline{C}_s$，我们沿用Mamba论文\cite{Mamba}的标准选择性机制，通过线性投影从输入内容动态生成，此处不再赘述。

\textbf{尺度约束机制：}ASD-SSM的核心创新在于通过尺度约束因子$\alpha_s$实现差异化的状态转移特性。该因子随尺度从细到粗递增（细尺度$\alpha_1 \approx 0.3$，粗尺度$\alpha_S \approx 0.9$），确保粗尺度保持更强的长程记忆能力，细尺度实现快速响应：
\begin{equation}
	\Delta\overline{A}_s = \alpha_s \cdot \text{LayerNorm}(\Delta\overline{A}_s^{raw})
\end{equation}
其中$\text{LayerNorm}(\cdot)$表示层归一化操作，用于稳定训练过程。最终的尺度特定状态转移参数通过向量加法获得：
\begin{equation}
	\overline{A}_s = \overline{A}_{base} + \lambda_A \cdot \Delta\overline{A}_s
	\label{eq:param_modulation}
\end{equation}
其中$\overline{A}_{base} \in \mathbb{R}^{D}$为所有尺度和Patch共享的基础状态转移参数向量（对角矩阵的对角元素），$\lambda_A$为调制强度系数（实验中设为0.1）。需要强调的是，虽然为简化表示公式中省略了Patch索引，但每个Patch $m$会根据其内容特征生成专属的参数偏移量$\Delta\overline{A}_s^{(m)}$，即完整形式为$\overline{A}_s^{(m)} = \overline{A}_{base} + \lambda_A \cdot \Delta\overline{A}_s^{(m)}$。

\textbf{Patch间状态传递策略：}为了充分利用序列化后的点云顺序信息同时保持对无序性的尊重，ASD-SSM采用Patch间状态传递策略。对于所有尺度$s$，同一batch内相邻Patch间传递隐状态：当处理第$m$个Patch时，其初始状态通过门控机制融合前一Patch的最终状态$\mathbf{h}_P^{(s,m-1)}$，即$\mathbf{h}_0^{(s,m)} = \text{Gate}(\mathbf{h}_P^{(s,m-1)}, \mathbf{u}_1^{(s,m)})$，动态吸收前序Patch的上下文信息。值得强调的是，该策略并未违背点云的无序性原则：在最底层（$s=1$）序列化后，我们对点云进行了有序化处理以构建空间邻接关系；而在更高尺度层（$s>1$），Patch内的点云被随机打乱，SSM在无序点集上进行特征学习，这一设计充分尊重了点云的无序特性。通过这种方式，模型既能利用序列化带来的空间邻接信息建立Patch间的连续性，又能在Patch内部保持对无序性的适应能力。全局上下文信息通过参数生成器中的全局特征提取（公式中的$\mathbf{f}_{global}^{(s)}$）、Patch间状态传递以及后续的多尺度特征融合共同保证。

这种设计使得ASD-SSM实现了尺度解耦的状态空间建模：（1）\textit{共享的基础记忆}——通过$\overline{A}_{base}$为所有尺度提供统一的状态转移基础；（2）\textit{尺度特定的记忆特性}——通过$\alpha_s$约束的$\Delta\overline{A}_s$实现粗尺度慢衰减（长程记忆）、细尺度快衰减（快速响应）的差异化建模，这是ASD-SSM的核心创新；（3）\textit{选择性信息流动}——通过Mamba的标准选择性机制\cite{Mamba}，$\overline{B}_s$和$\overline{C}_s$根据输入内容动态调整信息的选择性流入和流出。
...
\end{equation}
其中$\mathbf{h}_k^{(s,m)}$是尺度$s$、Patch $m$的隐状态向量，$\mathbf{u}_k^{(s,m)}$是Patch内第$k$个点的输入向量（$k=1,2,\ldots,P$），$\overline{D}$为直通连接参数。对于所有尺度，第$m$个Patch的初始状态通过门控机制融合前一Patch的信息：
\begin{equation}
	\begin{aligned}
	...
	\label{eq:state_gate}
\end{equation}
其中$\sigma$为Sigmoid函数，$\mathbf{g}^{(s,m)}$为门控权重，$\text{Proj}(\cdot)$为状态投影层，$\|$表示拼接操作，$\odot$表示逐元素相乘。门控权重$\mathbf{g}^{(s,m)}$基于当前Patch的首个输入$\mathbf{u}_1^{(s,m)}$和前一Patch的最终状态$\mathbf{h}_P^{(s,m-1)}$动态计算，自适应地决定保留多少前序上下文信息。这种设计使得模型能够根据当前输入和历史状态的相关性，自动调节状态传递的强度，避免了盲目累积不相关的历史信息。参数$\overline{A}_s, \overline{B}_s, \overline{C}_s$的具体维度和离散化细节遵循Mamba论文\cite{Mamba}的标准实现。值得强调的是，ASD-SSM的创新点在于$\overline{A}_s$的尺度自适应生成以及门控状态传递策略，而不是对SSM基本形式的改变。
...
为保证双向建模的一致性和参数效率，前向SSM和后向SSM共享同一组尺度特定的状态转移参数$\overline{A}_s$。前向SSM从第1个点到第$P$个点依次处理，后向SSM从第$P$个点到第1个点反向处理，最后通过残差连接融合双向特征。这种设计使得每个点都能同时获得前后文的上下文信息，同时保持了尺度自适应的状态转移特性——例如，粗尺度的$\overline{A}_s$（大的$\alpha_s$）在前向和后向传播中均保持长程记忆，而细尺度的$\overline{A}_s$（小的$\alpha_s$）在两个方向上都能快速响应局部变化。

通过公式\ref{eq:param_modulation}和\ref{eq:asd_ssm_update}，ASD-SSM实现了尺度解耦的状态空间建模：最细尺度（$s=1$）的$\overline{A}_1$（受较小的$\alpha_1$约束）实现快速状态衰减，能够快速响应局部细节变化；最粗尺度（$s=S$）的$\overline{A}_S$（受较大的$\alpha_S$约束）元素接近1（对应对角矩阵接近单位矩阵），保持长程记忆以捕捉全局语义。多层尺度特征提取完成后，我们采用拼接策略将不同尺度的特征$Points_{1:S}$进行逐点融合，最后通过MLP和线性层完成特征映射，将特征维度还原为初始大小$C$，得到融合多层次信息的点云表示$Points'\in\mathbb{R}^{N\times C}$。
...
从\cref{tab:paramgen_design}可以看出，仅使用尺度嵌入时性能提升0.7\% mIoU，证明了为不同尺度赋予独特标识的重要性。仅使用全局特征时提升0.9\% mIoU，说明内容自适应的参数生成更为关键。同时使用两者的提升（+1.8\%）大于单独使用之和（+0.7\% + +0.9\% = +1.6\%），展现出明显的协同效应。调制强度方面，$\lambda=0.1$时达到最优平衡，过小（0.05）导致$\overline{A}$的尺度差异化不足，过大（0.2, 0.3）则引入过多扰动，影响训练稳定性。
