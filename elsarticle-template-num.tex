...
性能提升主要源于两方面创新。首先，GGAM通过双序列化融合和几何特征增强，解决了序列化导致的几何结构丢失问题，这对边界区域（如墙与柱的交界）和复杂曲面的准确分割至关重要。其次，ASD-SSM通过自适应参数生成为不同尺度定制SSM参数，实现了全局语义和局部细节的有效平衡，而现有SSM方法采用共享参数的单一粒度特征提取，难以兼顾不同层次的信息需求。此外，PointSS继承了状态空间模型的线性时间复杂度特性，如\cref{tab:parameterization_comparison}所示，不同参数化方案的推理时间增加均不显著（<7\%），展现了良好的计算效率。
\begin{table}[htbp!]
...
如\cref{tab:progressive_improvement}所示，GGAM模块贡献+2.2\% mIoU，解决了序列化几何信息丢失问题；ASD-SSM模块在GGAM基础上进一步贡献+2.7\% mIoU，其中多尺度特征贡献+0.9\%，自适应参数生成贡献+1.8\%，实现了层次化的多尺度特征学习。
...
实验结果表明，GGAM模块贡献+2.2\% mIoU，ASD-SSM模块在GGAM基础上进一步贡献+2.7\% mIoU，完整PointSS相比基线总提升4.9\%。
...
实验结果表明，在Hilbert曲线序列化下，33\%的点存在不同程度的割裂问题（割裂近邻占比>10\%）。割裂程度与分割性能呈负相关，当割裂近邻占比从0-10\%增加到20-30\%时，分割IoU从74.0\%降至62.0\%，性能损失达12.0个百分点。GGAM通过显式注入几何先验有效补偿了这一问题，割裂程度越严重，GGAM的绝对提升越显著（从0.8\%增加到8.0\%），整体使平均IoU从70.3\%提升至72.5\%。
...
分析表明，GGAM的提升幅度与类别的平均割裂占比呈正相关。大平面类别（地板、墙、天花板）割裂占比较低（5-8\%），GGAM提升有限（+1.2-1.4\%）；中等复杂度家具类别（桌子、书架、椅子、沙发）割裂占比为11-15\%，提升为+3.1-4.3\%；边界密集类别（门、柱子、窗户）和复杂杂物的割裂占比升高（17-22\%），GGAM带来了更显著的性能改善（+5.3-6.5\%）。
...
组件贡献分析显示各模块呈现强协同效应。几何特征作为基础组件贡献+0.8\% mIoU。双序列化策略单独加入时性能轻微下降0.1\%，但配合交叉注意力后带来跳跃性提升，验证了双序列化与交叉注意力的强协同作用。门控融合和几何一致性损失各额外贡献+0.3\% mIoU。完整GGAM相比基线提升2.2\% mIoU。
...
实验结果显示，ASD-SSM在仅增加20\%参数量的情况下，性能接近独立参数方案（仅低0.6\% mIoU），参数效率显著更高。虽然参数插值也是轻量级方案，但简单的线性插值无法捕捉尺度间的非线性关系，性能提升有限（+0.6\% vs +1.8\%）。LoRA\cite{lora}通过低秩矩阵分解提供了一定的适配能力，但其静态的低秩约束限制了表达能力，而ASD-SSM的动态参数生成机制更灵活。值得注意的是，所有方法的推理时间增加均不明显（<7\%）。
...
从实验结果可以看出，添加第二个尺度即可带来显著提升（+1.1\%~+1.4\%），证明了多尺度建模的必要性。三尺度配置相比双尺度进一步提升1.6\%~1.9\% mIoU，说明三个层次（局部-中间-全局）能更好地捕捉点云的层次化结构。在窗口扩大策略方面，(1,2,2)配置优于(1,2,3)和(1,2,4)，表明过大的窗口会稀释空间特征信息。四尺度和五尺度的性能提升不明显（<0.2\%），但训练时间显著增加（>20\%），说明三尺度已达到较好的性能-效率平衡点。
