...
\textbf{多路径自适应序列化网络（MASA，第五章）}：作为相对独立的增强模块，MASA从序列化策略的角度进一步提升模型性能。与GGAM和ASD-SSM在固定序列化顺序下工作不同，MASA通过几何条件化的动态路由机制，根据局部几何特性自适应选择最适合的序列化路径。MASA可以视为对前两个创新点的进一步补充：GGAM和ASD-SSM解决了"在给定序列化下如何更好地学习"的问题，而MASA则解决了"如何选择更好的序列化"的问题。三者协同作用，构成了PointSS的完整技术体系。

从架构关系上看，三个创新点形成了"数据增强（GGAM） → 特征学习（ASD-SSM） → 序列化优化（MASA）"的递进关系，共同推动PointSS在点云分析任务中取得显著性能提升。本文后续章节将按照创新点的逻辑顺序，依次详细介绍GGAM（第三章）、ASD-SSM（第四章）和MASA（第五章）的设计思想、技术细节和实验验证。
