...
\textbf{关键观察}：虽然可以通过KNN精确计算几何特征并避免割裂问题，但其$O(N\log N)$的计算复杂度限制了模型规模和感受野的扩展，在大规模点云场景下成为性能瓶颈。因此我们采用窗口化邻域快速提取几何先验（$O(N)$复杂度），并通过GGAM(Global Geometry-Aware Mechanism)的全局几何感知机制显式注入几何信息，补偿Mamba的隐式几何学习不足，在效率与效果间取得更优平衡。
...
\subsection{三个核心创新点的关系与作用}

PointSS通过三个相互协同的核心创新点解决基于Mamba的点云分析方法中的关键问题，这三个创新点在架构中的位置和作用如下：

\textbf{全局几何感知机制（GGAM，第三章）}：针对序列化导致的几何信息丢失问题，GGAM作为特征增强模块位于数据处理阶段的末端，在点云进入编码器之前对其进行全局几何特征注入。通过窗口化邻域提取几何先验（曲率、法向量），并利用双序列化融合和全局交叉注意力机制，GGAM为后续的特征学习模块提供显式的几何感知能力，有效补偿了Mamba因序列化割裂导致的几何学习困难。GGAM是整个PointSS架构的基础模块，其输出的几何增强特征贯穿整个编码-解码过程。

\textbf{自适应尺度解耦状态空间模型（ASD-SSM，第四章）}：在GGAM解决几何丢失问题的基础上，ASD-SSM进一步解决长序列下特征粒度单一与因果关系强加的问题。ASD-SSM作为核心计算单元嵌入到编码器和解码器的每个stage中，通过尺度感知参数生成器为不同尺度动态生成定制化的SSM参数，使粗尺度倾向于捕获长程依赖，细尺度倾向于快速响应局部变化。ASD-SSM与GGAM紧密配合：GGAM提供的几何先验信息帮助ASD-SSM更好地理解点云的层次化结构，而ASD-SSM的多尺度建模能力则充分利用了GGAM注入的几何特征，实现了从局部细节到全局语义的层次化特征学习。

\textbf{多路径自适应序列化网络（MASA，第五章）}：MASA从全局序列化策略的角度与ASD-SSM形成互补。ASD-SSM虽然能在局部邻域内有效进行多尺度特征解耦,但其工作依赖于PTv3提供的固定序列化顺序,该顺序对所有场景采用相同的空间扫描方式,无法根据不同几何特性进行自适应调整。MASA通过几何条件化的动态路由机制,为不同几何特性的区域选择最适合的序列化路径:对于规则平面区域使用高效的空间填充曲线,对于复杂物体边界使用重要性优先的语义序列化。两者形成明确的功能分工:ASD-SSM负责\textit{局部层面}的多尺度特征解耦,在给定序列化下优化局部特征学习;MASA负责\textit{全局层面}的序列化路径选择,为ASD-SSM提供更优的全局点云顺序。消融实验表明,MASA使ASD-SSM的性能进一步提升1.7\%,两者协同效应显著,共同构成"全局序列化优化 + 局部尺度建模"的完整架构。

从架构关系上看,三个创新点形成了层次化的协同架构:GGAM作为基础模块,通过几何先验增强为后续的特征学习提供更丰富的输入表示;ASD-SSM和MASA形成"局部-全局"互补的双层特征学习架构,ASD-SSM负责局部邻域内的多尺度特征解耦,MASA负责全局层面的序列化路径优化,两者相互增强、缺一不可。消融实验表明,GGAM、ASD-SSM和MASA的性能提升存在显著的协同效应,组合使用的性能增益超过各模块单独贡献的简单相加,证明了三者的紧密配合。本文后续章节将按照创新点的逻辑顺序,依次详细介绍GGAM(第三章)、ASD-SSM(第四章)和MASA(第五章)的设计思想、技术细节和实验验证,并在第六章中通过综合实验展示三个模块的协同作用。
...
本章针对现有基于Mamba的点云分析方法在序列化过程中面临的几何信息丢失问题，提出了全局几何感知机制（GGAM）。GGAM通过显式提取和注入几何先验信息，有效补偿了Mamba在序列化场景下的隐式几何学习不足。

本章首先介绍了PointSS的整体架构，阐述了GGAM、ASD-SSM和MASA三个核心创新点的相互关系与协同作用。然后深入分析了几何丢失问题的根本原因：空间邻近的点在序列化后被割裂，Mamba的线性状态传递机制无法有效关联这些割裂点，导致模型难以隐式学习到完整的几何结构特征。

针对该问题，GGAM采用窗口化邻域快速提取几何先验（曲率和法向量），通过双序列化融合（Z-order和Hilbert编码）、全局交叉注意力机制和自适应门控融合策略，为后续的编码器和解码器提供丰富的几何感知能力。该设计在保持$O(N)$计算复杂度的同时，有效解决了序列化割裂导致的几何学习困难。

实验验证表明，GGAM相比单尺度双向Mamba基线提升2.2\% mIoU，对于割裂程度严重的点（割裂近邻占比20-30\%）带来了8.0\%的显著提升。不同语义类别的分析显示，GGAM在几何复杂的类别（如窗户、柱子、杂物）上的性能改善尤为显著（+5.6\%至+6.7\%），证明了显式几何先验对复杂边界和细节结构的有效补偿能力。核心组件消融实验验证了双序列化、交叉注意力、门控融合和几何一致性约束各模块的贡献，表明各模块之间存在强协同效应。

GGAM作为PointSS的基础模块，为后续的多尺度特征学习提供了坚实的几何感知基础，其输出的几何增强特征将贯穿整个编码-解码过程，与第四章的ASD-SSM和第五章的MASA共同构成PointSS的完整技术体系。
...
本章针对现有基于Mamba的点云分析方法在处理长序列点云时面临的特征提取粒度单一与因果关系强加问题，提出了自适应尺度解耦状态空间模型（ASD-SSM）。ASD-SSM通过为不同尺度动态生成定制化的状态空间模型参数，实现了层次化的多尺度特征学习，在尊重点云无序性的同时有效捕获了从局部细节到全局语义的层次化特征。

本章首先深入分析了问题动机：现有方法采用共享的状态空间参数处理所有尺度的特征，无法在邻域内精准区分远近点的学习权重，同时固定的序列化顺序为本质上无序的点云强加了因果关系，降低了模型的鲁棒性。针对这些问题，ASD-SSM提出通过尺度感知参数生成器为不同尺度动态生成定制化的SSM参数，使细尺度快速响应局部细节变化，粗尺度保持长程记忆捕获全局上下文。

ASD-SSM的核心设计包括三个方面：首先，通过多尺度窗口划分将点云序列划分为不同粒度的Patch，为多尺度特征学习建立基础；其次，设计尺度感知参数生成器，根据尺度索引动态生成状态转移矩阵$\overline{A}_s$、输入矩阵$\overline{B}_s$和输出矩阵$\overline{C}_s$的约束因子，实现尺度解耦的状态空间建模；最后，采用并行多尺度处理和特征融合策略，保持了线性时间复杂度的同时实现了丰富的层次化特征表示。

实验验证表明，ASD-SSM在GGAM基础上进一步贡献2.7\%的性能提升，使完整PointSS达到75.2\% mIoU。尺度约束因子$\alpha_s$的消融实验验证了尺度解耦机制的有效性，合理的$\alpha_s$配置（如细尺度0.3、粗尺度0.9）能够显著提升性能。参数化策略的对比实验显示，尺度感知参数生成方案在仅增加约20\%参数量的情况下，性能接近完全独立参数方案（仅低0.6\% mIoU），展现出卓越的参数效率。尺度数量的消融实验表明，三尺度配置达到了性能与效率的最优平衡点，能够有效捕捉点云的层次化结构。

ASD-SSM与第三章的GGAM形成紧密协同：GGAM提供的几何先验帮助ASD-SSM更好地理解点云的层次化结构，而ASD-SSM的多尺度建模能力则充分利用了GGAM注入的几何特征，实现了从局部细节到全局语义的层次化特征学习。两者共同构成了PointSS在编码器-解码器中的核心计算单元，为第五章MASA的全局序列化优化提供了坚实的局部特征学习基础。
