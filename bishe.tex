...
近年来，点云分析领域经历了从早期基于几何先验的方法到深度学习驱动的端到端模型的重要转变。研究人员针对点云数据的特殊性质，提出了一系列创新性解决方案。

\textbf{基于深度学习的点云语义分析。}早期的PointNet\cite{pointnet}作为开创性工作，通过对称函数（如最大池化）巧妙解决了点云无序性问题，设计了两级特征聚合架构实现端到端处理。然而其全局池化策略虽能捕捉整体形状信息，但在局部几何细节建模方面存在明显不足。PointNet++\cite{PointNet++}提出分层特征学习框架，通过"采样-分组-特征提取"策略，在不同尺度上递归应用PointNet结构，有效平衡了局部细节与全局结构的建模需求。随后，DGCNN\cite{DGCNN}引入动态图构建机制，在特征空间中定义邻域关系，显著增强了对复杂几何结构的适应能力。但基于卷积的方法仍受限于规则网格卷积和固定感受野，难以有效构建多尺度语义信息。

随着Transformer在自然语言处理和计算机视觉领域的成功，点云分析迎来了新的发展机遇。Point Transformer\cite{pt}作为先驱工作，设计了专门的点云自注意力模块，通过位置编码和特征增强策略，实现了对点云几何信息和语义特征的联合建模。PTv3\cite{ptv3}则采用简化设计理念，摒弃复杂的注意力交互机制，通过直接序列化策略取得了先进性能。尽管Transformer方法在全局建模能力上表现卓越，但标准自注意力机制的$O(N^2)$复杂度使得模型在面对大规模点云时难以保持实时性。

为解决Transformer的计算复杂度瓶颈，基于状态空间模型（SSM）的方法成为新兴方向。Mamba\cite{Mamba}通过选择性状态空间机制实现高效序列建模，其线性时间复杂度为大规模点云处理提供了理论基础。PointMamba\cite{PointMamba}首次将Mamba架构引入点云分析，通过空间填充曲线将三维点云序列化为一维序列，能够以线性复杂度实现全局信息聚合。PCM\cite{pcm}进一步优化了序列化策略，提出空间相邻性保持的点云排序方法。然而，现有SSM方法在将三维非结构化数据适配到一维序列处理框架时面临本质性挑战：其一，序列化导致空间邻近的点在序列中被割裂，使得法向量、曲率等几何特征难以被有效建模；其二，在长序列场景下无法为不同距离的邻域点动态分配差异化权重，导致特征表达的粒度单一，难以兼顾局部几何细节与跨尺度语义关联。

\textbf{基于几何特征增强的点云分析方法。}点云数据本质上携带丰富的几何信息，如局部表面法向量、曲率等，这些几何特征对于理解三维结构至关重要。PointGA\cite{pointga}通过三角位置编码机制，重构局部三角形结构来捕捉细粒度的几何关系。PointMSGT\cite{pointmsgt}提出多尺度几何特征提取框架，通过重构三角形结构提取详细的局部几何关系。然而，现有的几何特征增强方法主要面临两个挑战：一是计算效率问题，精确的几何特征计算（如基于k近邻的法向量估计）往往计算代价较高；二是特征融合问题，如何将几何特征与深度特征有效融合，避免信息冗余，仍然是一个开放性问题。

\textbf{基于多尺度特征融合的点云分析方法。}多尺度特征融合是点云分析中的核心技术之一。早期的工作通过多个并行分支提取不同尺度的局部特征，但这种方法往往存在特征冗余和计算开销大的问题。近年来的研究主要集中在设计更有效的自适应融合策略\cite{adaptive_fusion_2025,projection_based_fusion_2024}。但现有方法仍面临挑战：如何自动确定合适的尺度范围、如何设计自适应的权重学习机制、如何在保证性能的前提下优化计算效率。特别是在长序列点云处理中，现有方法难以在不同尺度间实现有效的信息传递和权重分配。

综上所述，目前基于Mamba的点云分析方法还有两个关键问题需要解决。第一，虽然序列化方法能在局部窗口内保持空间邻近性，但Mamba的线性状态传递机制无法有效关联空间邻近但序列距离较远的点（割裂点），导致模型难以综合所有空间邻居的几何信息，从而影响对复杂形状的精确学习，我们称这种情况为"几何丢失"。第二，当前的模型对于长序列下特征提取粒度单一，且为点云强加了因果关系，无法同时捕捉不同层次的特征信息，导致点云分析效果不佳。
...
第二章为相关工作，系统梳理基于深度学习的点云语义分析方法、几何特征增强方法和多尺度特征融合方法的研究进展，分析各方法的核心思想、技术贡献及其局限性。
