...
使用scatter操作向量化计算，复杂度为$O(|\mathcal{E}|D) = O(kND)$。对于$K$个频段，总复杂度为$O(KkND) = O(KND)$（$k$为常数）。

\subsubsection{频段特征的语义解释}

每个频段$T_k(\tilde{L})X$具有明确的几何语义：
\begin{itemize}
\item \textbf{$k=0$（直流分量）}：$T_0(\tilde{L})X = X$，保留原始特征，捕获点的固有属性
\item \textbf{$k=1$（低频）}：$T_1(\tilde{L})X = \tilde{L}X$，捕获一阶局部平滑，对应平缓区域（如墙面、地板）
\item \textbf{$k=2$（中频）}：$T_2(\tilde{L})X$，捕获二阶几何变化，对应曲面弯曲（如圆柱、球体）
\item \textbf{$k \geq 3$（高频）}：捕获高阶细节和边缘，对应物体边界、角点等几何突变
\end{itemize}

这种多频段分解自动将点云特征按几何复杂度进行了分层，为后续的差异化建模奠定了基础。
