...
前述章节提出的GGAM和ASD-SSM主要在空域进行特征学习，虽然取得了显著的性能提升，但仍存在局限：空域方法通过邻域聚合隐式捕获频率信息，难以显式地分离低频的全局结构和高频的局部细节，且无法对其进行差异化建模。

频域建模为解决上述问题提供了新的视角。通过频谱分解，点云特征可被分离为多个频段，其中低频分量对应全局平滑结构（如墙面、地板的整体形状），高频分量对应局部细节和边界（如物体交界处、细小结构）。这种显式的频率分离带来三方面优势：首先，能够针对不同频率分量采用差异化建模策略——对低频强化长程依赖以捕获全局一致性，对高频强化局部细节以保持边界清晰度；其次，避免了空域方法中全局与局部特征相互干扰的问题，提升了特征表达的纯净性；第三，频域与空域方法具有天然互补性，空域方法擅长几何邻域建模，频域方法擅长频率层级分析，两者结合能够更全面地刻画点云的多尺度特征。为此，本章从频域建模的角度，提出基于Chebyshev多项式近似的频谱状态空间模型。
