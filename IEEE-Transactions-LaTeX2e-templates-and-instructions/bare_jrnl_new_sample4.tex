...
Mamba-based point cloud analysis methods face difficulties in modeling local geometry structure due to the fundamental conflict between sequential state propagation and unordered 3D structure. Mamba processes sequences through the following discretized state space equations:
\begin{equation}
x_k = \bar{A}x_{k-1} + \bar{B}u_k, \quad y_k = \bar{C}x_k + \bar{D}u_k
\end{equation}
where $k$ denotes the position index in the serialized sequence, $x_k$ represents the state vector at position $k$, $u_k$ is the input feature vector, $y_k$ is the output feature vector, and $\bar{A}$, $\bar{B}$, $\bar{C}$, $\bar{D}$ are learnable parameters for state transition, input projection, output projection, and residual connection, respectively.
...
To address this issue, we propose the Global Geometry-Aware Mechanism (GGAM) to explicitly inject geometric priors into the model. As illustrated in \cref{fig:ggam}, GGAM consists of two parts: (1) Graph Construction and Geometric Feature Extraction, and (2) Geometric Feature Enhancement and Fusion. The first part constructs a local graph from the serialized point sequence and extracts a geometric feature vector for each point. To capture complementary spatial structure, this process runs in parallel on two serialization streams—Z-order encoding and Hilbert encoding—yielding two independent sets of geometric features. The second part enhances and fuses these dual-stream features through a cross-attention mechanism and an adaptive gating strategy.
...
Traditional methods for computing geometric relationships (e.g., curvature and surface normals) rely on K-Nearest Neighbor (KNN) searches, whose computational complexity is prohibitively high for large-scale point clouds. Instead, we propose a serialization-based approach that constructs local neighborhoods and computes geometric features directly from the serialized sequence.

Given the point cloud $\{p_1, p_2, \ldots, p_N\}$, let $\pi$ be the permutation induced by the serialization (e.g., Z-order or Hilbert curve). The serialized sequence is defined as $Q = (p_{\pi(1)}, p_{\pi(2)}, \ldots, p_{\pi(N)})$, where $q_i \triangleq p_{\pi(i)}$ denotes the $i$-th point in the serialized order. The sequence is then partitioned into non-overlapping windows:
\begin{equation}
W_m = (q_{(m-1) \cdot P + 1}, q_{(m-1) \cdot P + 2}, \ldots, q_{m \cdot P})
\end{equation}
where $m = 1, 2, \ldots, \lfloor N/P \rfloor$ is the window index and $P$ is the window size. If the last window has fewer than $P$ points, it is padded with points from the preceding window to maintain local continuity.

For any point $p_i$, let $\text{pos}(i)$ denote its position in the serialized sequence. Its local neighborhood is defined as the window it belongs to:
\begin{equation}
\mathcal{N}_P(p_i) = W_{\lfloor \frac{\text{pos}(i)-1}{P} \rfloor + 1}
\end{equation}

Within each window $W_m$, a fully connected graph is established over all points in the window:
\begin{equation}
\mathcal{E}_m = \{(q_i, q_j): q_i, q_j \in W_m, i \neq j\}
\end{equation}
Each node in this graph represents a point, and each edge connects a pair of co-window points. Compared to KNN, this construction reduces neighborhood complexity from $\mathcal{O}(N\log N)$ to $\mathcal{O}(N)$. This efficiency gain is not merely a compromise: Point Transformer V3 \cite{ptv3} showed that the computation saved by efficient neighborhood search can be reinvested to expand model capacity (e.g., enlarging the receptive field from 16 to 1024 points), ultimately outperforming KNN-based methods on multiple benchmarks.

For each edge $(q_i, q_j)$, an 8-dimensional geometric feature vector is extracted:
\begin{equation}
\mathbf{f}_{ij} = [\Delta\mathbf{p}_{ij},\ \alpha_{ij},\ \boldsymbol{\kappa}] \in \mathbb{R}^8
\end{equation}
where $\Delta\mathbf{p}_{ij} = \mathbf{p}_j - \mathbf{p}_i$ is the relative coordinate vector (3D). $\alpha_{ij}$ (3D) is a directional feature computed as the difference between the mean surface normal of the window and the normalized edge direction $\Delta\mathbf{p}_{ij}/\|\Delta\mathbf{p}_{ij}\|$; it measures how much the edge deviates from the local surface. $\boldsymbol{\kappa}$ (2D) encodes two curvature measures: the surface variation rate and the Gaussian curvature.

Each edge feature is passed through an MLP, and the results are aggregated per point via mean pooling over all incident edges:
\begin{equation}
\mathbf{H}_i = \frac{1}{|\mathcal{N}_P(p_i)|} \sum_{j:\,(i,j)\in\mathcal{E}} \text{MLP}(\mathbf{f}_{ij})
\end{equation}
The resulting feature $\mathbf{H}_i$ summarizes the local geometric context of point $p_i$. Since graph construction runs in parallel on Z-order and Hilbert serializations, each point obtains two independent geometric features, $\mathbf{H}_z$ and $\mathbf{H}_h$, which are passed to the second part of GGAM.
...
The two serialization encodings capture complementary spatial structures. Z-order encoding uses recursive binary partitioning, which produces strong local clustering and is well-suited for detecting geometric boundaries and local shape variations. Hilbert encoding provides better spatial continuity, helping to preserve the overall coherence of geometric structures across the sequence. Rather than treating these two views independently, we enrich each stream by letting it attend to the other.

Concretely, self-attention is first applied within each stream to aggregate local context:
\begin{equation}
A_z = \text{Attention}(Q_z, K_z, V_z), \quad A_h = \text{Attention}(Q_h, K_h, V_h)
\end{equation}
Cross-attention is then applied between the two streams, allowing each point's representation in one encoding to query the other:
\begin{equation}
A'_z = \text{Attention}(Q_z, K_h, V_h), \quad A'_h = \text{Attention}(Q_h, K_z, V_z)
\end{equation}
The self-attention and cross-attention outputs are combined to produce enhanced features $\mathbf{H}''_z$ and $\mathbf{H}''_h$, as detailed in \cref{fig:ggam}.
...
Since curvature and surface normals are estimated at window granularity rather than per point, they may carry noise from points near window boundaries. To prevent such noise from propagating into the fused features, a geometric consistency module is incorporated. It learns a soft gating factor $\boldsymbol{\gamma} \in [0, 1]$ that selectively suppresses unreliable feature components:
\begin{align}
...
The output $\mathbf{H}_{\text{final}}$ retains feature components that are consistent with the underlying geometric structure while attenuating potentially noisy ones, and is passed to subsequent layers as the geometry-aware point feature.
