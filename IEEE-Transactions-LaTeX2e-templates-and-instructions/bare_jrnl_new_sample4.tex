...
To further refine the fused features, a geometric consistency constraint module evaluates the agreement between the two streams: when the dual-stream features are consistent, the gate preserves the fused representation; when they exhibit significant discrepancy—indicating potentially unreliable geometric combinations introduced by window-approximated neighborhoods—the gate selectively suppresses the output. Specifically, a learnable scalar gate $\gamma \in [0,1]$ is derived from the concatenation of both streams:
\begin{equation}
