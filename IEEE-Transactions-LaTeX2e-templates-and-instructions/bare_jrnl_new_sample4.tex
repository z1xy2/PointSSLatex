...
\begin{table*}[!t]
	...
	\begin{tabular}{llllll}
		\toprule
		\textbf{Proximity} & \textbf{Point} & \textbf{Point} & \textbf{Baseline} & \textbf{GGAM} & \textbf{Absolute} \\
		\textbf{Loss Ratio} & \textbf{Count} & \textbf{Percentage} & \textbf{IoU (\%)} & \textbf{IoU (\%)} & \textbf{Gain (\%)} \\
		\midrule
	...
	\end{tabular}
	\par\vspace{3pt}
	\footnotesize Note: The proximity loss ratio is defined as the number of points among a point's K-nearest neighbors whose position in the serialized sequence is more than 300 apart from the center point, divided by K.
\end{table*}
\subsubsection{Ablation Study of GGAM}

We verify the effectiveness of GGAM through three experiments on the S3DIS dataset.

To measure the degree of spatial proximity loss after serialization, we search for 128 spatial neighbors for each point (referred to as the center point), and compute the proportion of these neighbors whose position in the serialized sequence is more than 300 apart from the center point. This ratio, termed the proximity loss ratio, is used to group points by their degree of spatial proximity loss and compare segmentation performance between the baseline and GGAM (\Cref{tab:ggam_fragmentation}).

Experimental results indicate that approximately 33\% of points suffer from the loss of spatial proximity under Hilbert serialization. As the proximity loss ratio increases from 0--10\% to 20--30\%, baseline performance drops sharply from 74.0\% to 62.0\%, confirming the detrimental impact of serialization on local geometric representation. GGAM effectively mitigates this degradation, with larger gains at higher proximity loss levels. Notably, an absolute gain of 8.0\% is observed in the high proximity loss interval (20--30\%), significantly exceeding improvements in low proximity loss areas.



This trend is further corroborated by category-wise analysis (\Cref{tab:ggam_category}). Large planar structures with high continuity (e.g., wall, floor) show limited gains (1.0--1.4\%). In contrast, categories featuring complex boundaries and slender structures (e.g., column, clutter) exhibit higher proximity loss ratios and achieve substantial improvements (5.3--6.5\%). These findings demonstrate that GGAM is particularly effective at mitigating the loss of spatial proximity in regions with sharp transitions and dense boundaries.
...
\begin{tabular}{llllll}
\toprule
\textbf{Category} & \textbf{Avg. Proximity Loss Ratio} & \textbf{Baseline IoU (\%)} & \textbf{GGAM IoU (\%)} & \textbf{Absolute Gain (\%)} & \textbf{Geometric Characteristics} \\
\midrule
