...
\subsubsection{Motivation and Overall Scheme}

Existing multi-scale feature extraction methods employ shared state space parameters when processing point clouds at different scales, overlooking the differentiated requirements of temporal modeling across scales. Coarse-scale features necessitate long-term memory to capture global context, while fine-scale features require rapid state updates to respond to local structural variations. To address this issue, as well as the limitations of SSM-based methods in handling point cloud disorder and long sequences, the Adaptive Scale-Decoupled State Space Model (ASD-SSM) is proposed.
