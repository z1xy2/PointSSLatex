...
The baseline model utilizes a standard single-scale bidirectional Mamba. As shown in \Cref{tab:progressive_improvement}, introducing GGAM alone yields a +2.2\% mIoU improvement, confirming that extracting geometric priors through windowed neighborhoods and dual-serialization effectively mitigates serialization-induced spatial proximity loss. Building on this, ASD-SSM with scale-aware parameter generation contributes a further +2.7\%, demonstrating that scale-decoupled parameterization is crucial for simultaneously capturing local details and modeling global semantics. Together, the two modules address distinct limitations and yield a cumulative gain of 4.9\% mIoU over the baseline.
...
Category-wise analysis (\Cref{tab:ggam_category}) further confirms this trend at the category level. Large planar structures (e.g., floor, wall) experience less serialization-induced loss of spatial proximity and exhibit limited gains. As the proximity loss ratio increases, the compensation effect of GGAM becomes more pronounced: categories with complex boundaries or irregular structures (e.g., window, door, board, clutter) show substantially larger improvements. These findings confirm that GGAM provides more pronounced compensation for serialization-induced proximity loss in structurally complex regions.
...
\begin{table*}[!t]
	...
	\end{tabular}
	\par\vspace{3pt}
	\footnotesize Note: The Avg. Proximity Loss Ratio is defined as the mean proximity loss ratio across all points belonging to a specific semantic category. The beam category is excluded from trend analysis, as beam instances are extremely sparse in S3DIS Area 5, yielding near-zero recall for both models.
\end{table*}
