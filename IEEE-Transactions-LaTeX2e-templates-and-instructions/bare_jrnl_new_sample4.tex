...
The difficulty in geometric learning within Mamba-based point cloud analysis methods stems from the inherent mismatch between state space model mechanisms and 3D geometric structures. Mamba performs sequence modeling through the following discretized state space equations:
\begin{equation}
...
Under this recurrent formulation, the state vector $x_k$ at each position is computed based solely on the previous state $x_{k-1}$ and the current input $u_k$. As discussed in Section I, serialization disrupts spatial proximity, causing spatially adjacent points to become sequentially distant. Consequently, the recurrent state propagation cannot effectively establish associations across these scattered neighbors, limiting the model's ability to capture complete geometric structures.
