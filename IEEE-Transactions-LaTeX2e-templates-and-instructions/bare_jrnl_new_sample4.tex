...
The limitations of existing methods stem from coupling serialization strategies with linear modeling. First, to enhance the local-preserving property of serialization, existing methods introduce structured preprocessing (e.g., space-filling curves with fixed traversal orders). However, such deterministic ordering inevitably causes models to learn order-dependent patterns, making them sensitive to input permutations---when the point cloud arrangement changes, significant discrepancies emerge in the output. Second, within the linear state space framework, it is difficult to assign adaptive learning weights to points with varying spatial distances or structural relationships within the same neighborhood. This limitation prevents the model from simultaneously balancing fine-grained geometric detail capture from proximate points and high-level semantic aggregation from distant contexts, resulting in single-granularity feature representation and insufficient hierarchical consistency, particularly in large-scale complex scenes.
