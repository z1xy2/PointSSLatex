...
point cloud serialization. This design prioritizes
...
The overall architecture of the PointSS model is illustrated in \cref{fig:architecture}. The data processing stage encompasses data augmentation and , where point clouds are serialized following PTv3 \cite{ptv3}. Following data processing, we employ the Global Geometry-Aware Mechanism (GGAM) to capture global geometric priors.
The feature learning component utilizes a PointNet-based U-shaped encoder-decoder architecture. Note that some points may appear in multiple patches (highlighted in orange) to maintain uniform patch sizes. Finally, the fused features are fed into a prediction head. The ultimate result, whether for individual points or the entire point cloud, is determined by selecting the category with the highest probability. Compared to PCM, this scheme only requires point cloud serialization at the beginning of training. It performs down-sampling directly via pooling on the serialized point cloud, rather than executing multiple serializations and using K-Nearest Neighbor (KNN) for neighborhood search and down-sampling within the encoder. This approach significantly reduces computational complexity.
