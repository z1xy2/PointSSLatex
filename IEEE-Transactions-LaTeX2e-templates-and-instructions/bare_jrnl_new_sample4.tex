...
Each stream is first normalized via LayerNorm before computing attention, where $\text{Attention}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{Softmax}(\mathbf{Q}\mathbf{K}^\top / \sqrt{d})\mathbf{V}$. Self-attention is then applied within each normalized stream:
\begin{equation}
\begin{aligned}
