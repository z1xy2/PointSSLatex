...
\begin{equation}
\mathbf{f}_{ij} = [\Delta\mathbf{q}_{ij},\ \mathbf{d}_{ij},\ \boldsymbol{\kappa}] \in \mathbb{R}^8
\end{equation}
where $\mathbf{q}_i \in \mathbb{R}^3$ denotes the 3D coordinates of point $q_i$, and $\Delta\mathbf{q}_{ij} = \mathbf{q}_j - \mathbf{q}_i \in \mathbb{R}^3$ is the relative position vector. $\mathbf{d}_{ij} \in \mathbb{R}^3$ is a surface-deviation feature measuring how much the edge direction deviates from the local surface:
\begin{equation}
\mathbf{d}_{ij} = \bar{\mathbf{n}}_m - \frac{\Delta\mathbf{q}_{ij}}{\|\Delta\mathbf{q}_{ij}\|}
\end{equation}
