...
achieves 9--19\% faster inference and remains operational at large patch sizes where
...
analysis. The main contributions of this work are as follows:
  ...
  \begin{enumerate}
	...
	semantic segmentation on S3DIS (74.1\% mIoU) and outperforms all SSM-based
...
\cref{tab:s3dis} summarizes the semantic segmentation performance on the S3DIS dataset. Without pre-training, PointSS achieves 74.1\% mean Intersection over Union (mIoU) on Area 5, outperforming all methods in the comparison. Specifically, it surpasses the state-of-the-art Point Transformer V3 by 0.7\% and the SSM-based Pamba by 0.6\%. Furthermore, PointSS retains the linear time complexity of SSMs. As shown in \cref{fig:efficiency}, compared to PTv3 under identical FP32 precision, PointSS achieves approximately 9--19\% reduction in inference time across patch sizes from 128 to 512. At patch size 1024, PTv3 encounters out-of-memory errors, while PointSS continues to operate normally, demonstrating the scalability advantage of linear SSM complexity over quadratic attention.
...
Swin3D\cite{Swin3D} & CVMJ'25 & Transformer & 72.5 \\
...
Pamba\cite{pamba} & AAAI'25 & SSM & 73.5 \\
HybridTM\cite{hybridtm} & IROS'25 & SSM & 72.1 \\
PointMSGT\cite{pointmsgt} & Mathematics'24 & Transformer & 68.6 \\
PointGA\cite{pointga} & Sci. Rep.'25 & Transformer & 66.2 \\
PointSS (Ours) & -- & SSM & \textbf{74.1} \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[!t]
	...
	\begin{tabular}{lllll}
		...
		\midrule
		Floor     & 5\%    & 97.3  & 97.5  & +0.2 \\
		Wall      & 6\%    & 84.4  & 84.8  & +0.4 \\
		Ceiling   & 8\%    & 89.9  & 90.3  & +0.4 \\
		Table     & 11\%   & 80.8  & 81.5  & +0.7 \\
		Bookshelf & 13\%   & 78.5  & 80.0  & +1.5 \\
		Chair     & 15\%   & 91.1  & 91.6  & +0.5 \\
		Sofa      & 15\%   & 81.5  & 84.3  & +2.8 \\
		Board     & 15\%   & 84.8  & 86.9  & +2.1 \\
