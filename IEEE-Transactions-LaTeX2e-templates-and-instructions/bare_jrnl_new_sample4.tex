...
Scale-aware Parameter Generator: ASD-SSM adaptively generates scale-specific state transition parameters $\bar{A}_s$ for different scales. For a given scale $s$, the point cloud features are partitioned into patches as $\mathbf{P}_s \in \mathbb{R}^{M \times P \times C}$, where $M = \lceil N/P \rceil$ is the number of patches. The parameter generation process is conducted independently for each patch, producing a patch-specific $\bar{A}_s$ conditioned on the content features of that patch.

Specifically, we first apply global average pooling over the $P$ points within each patch to obtain the patch-level feature $\mathbf{f}_{\text{global}}^{(s)} \in \mathbb{R}^{C/4}$, which captures the overall geometric characteristics of the patch (e.g., local structural complexity and density distribution). Simultaneously, we learn a scale embedding vector $\mathbf{e}_s \in \mathbb{R}^{C/4}$ for each scale to encode scale-specific information. For a single patch, the parameter generation process is as follows:
\begin{align}
...
\end{align}
where $\mathbf{h}_k^{(s,m)}$ is the hidden state vector for scale $s$ and patch $m$, $\mathbf{u}_k^{(s,m)}$ is the input vector of the $k$-th point within the patch ($k = 1,2, \ldots, P$), and $\bar{D}$ denotes the skip connection parameter. For the $m$-th patch, the initial state is set as $\mathbf{h}_0^{(s,m)} = \mathbf{h}_P^{(s,m-1)}$ (initialized as a zero vector for the first patch). This cross-patch state propagation ensures that state transitions at patch boundaries remain coherent with those within the patches.
