...
\begin{abstract}
State Space Models (SSMs) offer linear computational complexity for point cloud analysis but suffer from spatial proximity loss when serializing 3D point clouds into 1D sequences. To address this limitation, we propose PointSS, a geometry-aware multi-scale state space framework. We introduce a Global Geometry-Aware Mechanism (GGAM) that injects explicit geometric priors through windowed k-nearest neighbor graphs constructed on dual serializations (Z-order and Hilbert curves). Edge features encoding surface normals, curvature, and angular deviations are aggregated via cross-sequence attention and gated fusion, compensating for serialization-induced spatial discontinuities. Building on these geometric priors, we design an Adaptive Scale-Decoupled State Space Model (ASD-SSM) that adapts the state transition parameter $\bar{A}$ to local geometric characteristics via input-dependent generation. Scale constraint factors $\alpha_s$ control state decay rates across hierarchical scales: fine scales ($\alpha_s = 0.5$) enable rapid decay for local geometric detail, while coarse scales ($\alpha_s = 2.0$) allow slow decay for global semantic context. On S3DIS Area 5, PointSS achieves 73.8\% mIoU, outperforming Point Transformer V3 (73.4\%) and SSM-based methods including Pamba (73.5\%) and Point Cloud Mamba (72.6\%). On ModelNet40, it achieves 94.6\% overall accuracy. Compared to Point Transformer V3, PointSS achieves 9--19\% faster inference while maintaining linear complexity.
\end{abstract}
