...
\begin{figure*}[htbp]
	\centering
	\subfloat[]{\includegraphics[width=0.48\linewidth]{picture/panel_a.png}}
	\hfill
	\subfloat[]{\includegraphics[width=0.165\linewidth]{picture/panel_b.png}}
	\hfill
	\subfloat[]{\includegraphics[width=0.165\linewidth]{picture/panel_c.png}}
	\hfill
	\subfloat[]{\includegraphics[width=0.165\linewidth]{picture/panel_d.png}}
	\caption{Motivation and segmentation comparison on an indoor scene from S3DIS. \textbf{(a)} Left: RGB point cloud. Right: points colored by their 1D Hilbert serialization rank (blue $\to$ red = sequence start $\to$ end), revealing that spatially adjacent points at geometric boundaries can be separated by a large rank gap in the 1D sequence. \textbf{(b)} Ground truth semantic labels. \textbf{(c)} Prediction of the baseline without GGAM (w/o GGAM), with a zoom-in (red box) highlighting boundary confusion caused by serialization-induced proximity loss. \textbf{(d)} Prediction of our PointSS, with the same region (green box) showing sharp, accurate boundaries enabled by explicit geometric prior injection via GGAM.}
	\label{fig:analysis}
\end{figure*}
