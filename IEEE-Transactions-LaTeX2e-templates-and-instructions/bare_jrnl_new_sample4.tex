...
The baseline model utilizes a standard single-scale bidirectional Mamba. As shown in \Cref{tab:progressive_improvement}, introducing GGAM alone yields a 2.2\% mIoU improvement. This confirms that extracting geometric priors through windowed neighborhoods and dual-serialization effectively mitigates the serialization-induced loss of spatial proximity, thereby enhancing local structural representation.

Building on this, employing ASD-SSM with scale-aware parameter generation boosts performance to 75.2\%, providing an additional 2.7\% gain. This demonstrates that scale-decoupled parameterization is crucial for simultaneously capturing local details and modeling global semantics.

GGAM and ASD-SSM are complementary by design, together yielding a total improvement of 4.9\% mIoU.
...
Experimental results indicate that approximately 33\% of points suffer from the loss of spatial proximity under Hilbert serialization. As this proportion increases from 0--10\% to 20--30\%, baseline performance drops sharply from 74.0\% to 62.0\%, confirming the detrimental impact of serialization on local geometric representation. GGAM effectively mitigates this degradation, with larger gains at higher levels of spatial proximity loss. Notably, an absolute gain of 8.0\% is observed in the high proportion group (20--30\%), significantly exceeding improvements in the low proportion group.



Category-wise analysis (\Cref{tab:ggam_category}) shows that categories with more severe spatial proximity loss also benefit more from GGAM. Large planar structures with high continuity (e.g., wall, floor) show limited gains (1.0--1.4\%). In contrast, categories featuring complex boundaries and slender structures (e.g., column, clutter) exhibit a higher proportion of neighbors far apart in the serialized sequence and achieve substantial improvements (5.3--6.5\%). These findings demonstrate that GGAM is particularly effective at mitigating the loss of spatial proximity in regions with sharp transitions and dense boundaries.
...
Table & 11\% & 73.5 & 76.6 & +1.7 & Rectangular surface with legs \\
Bookshelf & 13\% & 70.2 & 73.9 & +1.5 & Vertical structure \\
Chair & 15\% & 68.9 & 73.2 & +2.3 & Complex structure \\
Sofa & 15\% & 70.6 & 74.8 & +2.8 & Curved surface with backrest \\
Door & 17\% & 65.2 & 70.5 & +2.3 & Rectangular boundaries \\
Column & 18\% & 54.3 & 60.1 & +3.8 & Slender vertical structure \\
Window & 19\% & 61.8 & 67.3 & +4.0 & Complex boundaries \\
Clutter & 22\% & 45.7 & 52.2 & +3.9 & Highly complex \\
\midrule
...
We further ablate the constituent components of GGAM (\Cref{tab:ggam_components}) to verify their individual contributions and synergistic effects.

Geometric features alone lead to a 0.8\% mIoU gain, suggesting that explicit geometric priors provide a more reliable foundation for modeling local structural information. Adding dual-serialization without an interaction mechanism yields limited improvement or even slight fluctuation, as different serialization methods may introduce directional biases without an effective fusion strategy.

Furthermore, combining geometric features with dual-serialization yields 71.0\% mIoU, indicating that multiple serialization sequences can provide complementary neighborhood information to a certain extent.

Adding cross-sequence attention further improves performance to 71.9\% mIoU, demonstrating that cross-sequence interaction plays a pivotal role in aligning complementary geometric information across different serialization patterns. Incorporating the gated fusion mechanism and geometric consistency constraint further elevates performance to 72.5\% mIoU, as the gated mechanism adaptively balances contributions from different serialization branches while the consistency constraint suppresses unreasonable geometric combinations caused by window-approximated neighborhoods.

The overall 2.2\% mIoU gain demonstrates that these components work synergistically to mitigate the serialization-induced loss of spatial proximity.
