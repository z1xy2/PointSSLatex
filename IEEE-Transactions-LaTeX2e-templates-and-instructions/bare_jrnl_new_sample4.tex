...
\end{equation}
$\cos\theta_i$ measures how much the edge direction $\Delta q_{ij}$ deviates from the tangent plane at $q_i$: it is near zero on smooth regions and grows large near geometric edges or creases. $\cos\theta_j$ captures the same quantity from the perspective of $q_j$. $\kappa_i, \kappa_j \in \mathbb{R}$ denote the surface curvatures at the two endpoints.
...
Each stream is first normalized via LayerNorm before computing attention. Self-attention is then applied within each normalized stream:
\begin{equation}
\begin{aligned}
A_z = \text{Attention}(\mathbf{H}_z, \mathbf{H}_z, \mathbf{H}_z), \\
A_h = \text{Attention}(\mathbf{H}_h, \mathbf{H}_h, \mathbf{H}_h)
\end{aligned}
\end{equation}
with residual connections:
\begin{equation}
\mathbf{H}'_z = \mathbf{H}_z + A_z, \quad \mathbf{H}'_h = \mathbf{H}_h + A_h
\end{equation}
...
To further refine the fused features, a geometric consistency module introduces a learnable scalar gate $\gamma \in [0,1]$ that adaptively controls the overall magnitude of $\mathbf{H}_{\text{fused}}$:
\begin{equation}
