...
Recently, State Space Models (SSMs) \cite{ssm,Mamba}, exemplified by PointMamba \cite{PointMamba} and PCM \cite{pcm}, have gained attention due to their linear complexity. Despite efficiency, serializing 3D point clouds into
1D sequences results in the loss of spatial proximity. The root of geometric loss lies in the conflict between Mamba's linear state transition and 3D spatial proximity. Serialization using space-filling curves inevitably creates ``fracture points"---neighbors that are spatially close but sequentially distant. In Mamba's recurrent mechanism, information from these distant points decays before reaching the center point, preventing the effective recovery of local attributes like surface normals and curvature. As illustrated in \cref{fig:analysis}, this information loss is particularly detrimental at object boundaries (e.g., wall-pillar junctions), where spatially adjacent yet sequentially distant context is crucial for accurate segmentation.
