...
\begin{equation}
\mathbf{f}_{ij} = [\Delta q_{ij},\ \mathbf{d}_{ij},\ \boldsymbol{\kappa}] \in \mathbb{R}^8
\end{equation}
where $\Delta q_{ij} = q_j - q_i \in \mathbb{R}^3$ is the relative position vector. $\mathbf{d}_{ij} \in \mathbb{R}^3$ is a surface-deviation feature measuring how much the edge direction deviates from the local surface:
\begin{equation}
\mathbf{d}_{ij} = \bar{\mathbf{n}}_m - \frac{\Delta q_{ij}}{\|\Delta q_{ij}\|}
\end{equation}
