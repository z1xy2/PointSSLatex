...
Given the point cloud $\{p_1, p_2, \ldots, p_N\}$, the serialization method sorts these points into an ordered sequence according to a space-filling curve. For example, a Z-order serialization of five points might yield $Q = (p_3, p_5, p_2, p_1, p_4)$, where the position of each point in $Q$ corresponds to its traversal position along the curve. In general, let $\pi$ be the permutation induced by the serialization. The serialized sequence is defined as:
\begin{equation}
...
For each point $q_i$, its neighborhood $\mathcal{N}(q_i)$ is the set of all other points within the same window:
\begin{equation}
\mathcal{N}(q_i) = \bigl\{q_j : q_j \in W_m,\ j \neq i\bigr\}, \quad m = \left\lceil \frac{i}{P} \right\rceil
\end{equation}

Within each window $W_m$, a fully connected graph is established over all co-window points:
\begin{equation}
\mathcal{E}_m = \{(q_i, q_j): q_i, q_j \in W_m,\ i \neq j\}
\end{equation}
Each node represents a point, and each edge connects a pair of points within the same window. Compared to KNN, this construction reduces neighborhood complexity from $\mathcal{O}(N\log N)$ to $\mathcal{O}(N)$. This efficiency gain is not merely a compromise: Point Transformer V3 \cite{ptv3} showed that the computation saved by efficient neighborhood search can be reinvested to expand model capacity (e.g., enlarging the receptive field from 16 to 1024 points), ultimately outperforming KNN-based methods on multiple benchmarks.

For each edge $(q_i, q_j) \in \mathcal{E}_m$, an 8-dimensional geometric feature vector is extracted:
\begin{equation}
\mathbf{f}_{ij} = [\Delta\mathbf{p}_{ij},\ \mathbf{d}_{ij},\ \boldsymbol{\kappa}] \in \mathbb{R}^8
\end{equation}
where $\Delta\mathbf{p}_{ij} = \mathbf{p}_j - \mathbf{p}_i \in \mathbb{R}^3$ is the relative position vector. $\mathbf{d}_{ij} \in \mathbb{R}^3$ is a surface-deviation feature measuring how much the edge direction deviates from the local surface:
\begin{equation}
\mathbf{d}_{ij} = \bar{\mathbf{n}}_m - \frac{\Delta\mathbf{p}_{ij}}{\|\Delta\mathbf{p}_{ij}\|}
\end{equation}
where $\bar{\mathbf{n}}_m$ is the mean surface normal estimated over window $W_m$. $\boldsymbol{\kappa} \in \mathbb{R}^2$ encodes two window-level curvature measures: the surface variation rate and the Gaussian curvature.

Each edge feature is passed through an MLP, and the outputs are aggregated per point via mean pooling over its neighborhood:
\begin{equation}
\mathbf{H}_i = \frac{1}{|\mathcal{N}(q_i)|} \sum_{q_j \in \mathcal{N}(q_i)} \text{MLP}(\mathbf{f}_{ij})
\end{equation}
The resulting feature $\mathbf{H}_i \in \mathbb{R}^C$ encodes the local geometric context of $q_i$. Since graph construction runs in parallel on Z-order and Hilbert serializations, each point obtains two independent geometric feature vectors, $\mathbf{H}_z$ and $\mathbf{H}_h$, which are passed to the second part of GGAM.
...
The two serializations capture complementary spatial structures. Z-order encoding uses recursive binary partitioning, producing strong local clustering that is well-suited for detecting geometric boundaries and local shape variations. Hilbert encoding preserves better spatial continuity, maintaining the overall coherence of geometric structures. Rather than treating these two views independently, we enrich each stream by letting it attend to the other.

Let $Q_z, K_z, V_z$ denote the query, key, and value projections of $\mathbf{H}_z$, and $Q_h, K_h, V_h$ those of $\mathbf{H}_h$. Self-attention is first applied within each stream:
\begin{equation}
A_z = \text{Attention}(Q_z, K_z, V_z), \quad A_h = \text{Attention}(Q_h, K_h, V_h)
\end{equation}
Cross-attention is then applied between the two streams, where each stream uses its own queries but attends to the keys and values of the other:
\begin{equation}
A'_z = \text{Attention}(Q_z, K_h, V_h), \quad A'_h = \text{Attention}(Q_h, K_z, V_z)
\end{equation}
The self-attention and cross-attention outputs are combined via residual addition and layer normalization:
\begin{equation}
\mathbf{H}''_z = \text{LayerNorm}(\mathbf{H}_z + A_z + A'_z), \quad \mathbf{H}''_h = \text{LayerNorm}(\mathbf{H}_h + A_h + A'_h)
\end{equation}
