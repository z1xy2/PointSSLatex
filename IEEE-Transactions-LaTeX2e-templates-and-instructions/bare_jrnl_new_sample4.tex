...
\author{Xinyuan Zhang, Xin Wang,~\IEEEmembership{Member,~IEEE}
\thanks{X. Zhang and X. Wang are with Jilin University, Changchun, China.}
...
\begin{abstract}
Point clouds, as discrete representations of three-dimensional space, have wide applications in autonomous driving, robotic navigation, and virtual reality. Accurate extraction of geometric features and semantic information from point clouds is crucial for downstream tasks. However, the inherent unordered, sparse, and irregular nature of point cloud data poses significant challenges for feature extraction. Traditional convolution-based methods struggle to directly handle the unstructured characteristics of point clouds, while Transformer-based methods, though capable of capturing global dependencies, are limited by their quadratic complexity in large-scale point cloud scenarios.

Recently, the Mamba model (an efficient state space model) from natural language processing has been widely applied to point cloud analysis due to its linear complexity and strong sequence modeling capabilities. However, applying Mamba to point clouds still faces challenges: the inherent contradiction between the unordered nature of point clouds and Mamba's requirement for sequential input necessitates serialization, which separates spatially adjacent points in the sequence, leading to geometric information loss. Since key geometric structures are typically defined by spatial nearest neighbors, this geometric loss makes it difficult for Mamba's linear state propagation mechanism to implicitly learn complete geometric structural features. Additionally, in long-sequence multi-scale feature learning, models typically cannot distinguish the importance weights of neighboring and distant points, failing to precisely capture local details and global high-level information. Meanwhile, serialization imposes unnecessary causal relationships on inherently unordered point clouds.

To address these issues, we propose PointSS, a point cloud feature extraction neural network based on Adaptive Scale-Decoupled State Space Models. To tackle the geometric learning difficulty, we design a Global Geometry-Aware Mechanism (GGAM) that efficiently extracts geometric priors (curvature and normals) through windowed neighborhoods, and explicitly injects geometric information into Mamba via dual serialization fusion and global cross-attention mechanisms, effectively compensating for its geometric learning deficiency caused by sequence fragmentation and enhancing the model's ability to capture point cloud geometric structures.

To address feature granularity limitations and imposed causality in long sequences, we propose the Adaptive Scale-Decoupled State Space Model (ASD-SSM). ASD-SSM dynamically generates customized SSM parameters for different scales through a lightweight parameter generator, enabling coarse scales to capture long-range dependencies while fine scales quickly respond to local variations. This approach achieves hierarchical multi-scale feature learning while respecting point cloud disorder. With only approximately 20\% additional parameters, this method achieves performance close to fully independent parameter schemes (only 0.6\% lower mIoU), significantly improving parameter efficiency and expressive power.

Experimental results demonstrate that the proposed algorithm has clear advantages over current mainstream methods and state space-based point cloud analysis approaches. Without using pre-training, PointSS achieves 75.2\% mean Intersection over Union (mIoU) on the S3DIS indoor segmentation benchmark, surpassing the best-performing similar model PCM by 5.4\% mIoU. On the ModelNet40 3D classification benchmark, it surpasses the best-performing similar model Mamba3D by 1.9\% in overall accuracy, reaching 96.0\% accuracy.
\end{abstract}
...
\caption{Visualization of geometric structure loss after serialization.}
\label{fig:analysis}
\end{figure}
...
\caption{Overall architecture of PointSS.}
...
\caption{Detailed flow of ASD-SSM.}
\label{fig:bss}
\end{figure}
...
\toprule
\begin{tabular}[c]{@{}l@{}}Fragmented\\Neighbor Ratio\end{tabular} & \begin{tabular}[c]{@{}l@{}}Point\\Count\end{tabular} & \begin{tabular}[c]{@{}l@{}}Point\\Ratio\end{tabular} & \begin{tabular}[c]{@{}l@{}}Baseline\\IoU\end{tabular} & \begin{tabular}[c]{@{}l@{}}GGAM\\IoU\end{tabular} & \begin{tabular}[c]{@{}l@{}}Absolute\\Gain\end{tabular} & \begin{tabular}[c]{@{}l@{}}Compensation\\Rate\end{tabular} \\
\midrule
0-10\% (Good) & 270K & 67\% & 74.0 & 74.8 & +0.8 & - \\
\midrule
10-20\% (Light) & 101K & 25\% & 63.0 & 67.1 & +4.1 & 37.3\% \\
20-30\% (Moderate) & 32K & 8\% & 62.0 & 70.0 & +8.0 & 66.7\% \\
\midrule
\textbf{Total} & \textbf{403K} & \textbf{100\%} & \textbf{70.3} & \textbf{72.5} & \textbf{+2.2} & \textbf{-} \\
\bottomrule
\end{tabular}
\end{table}

Note 1: Fragmented neighbor ratio = number of K nearest neighbors with sequence distance $>500$ / K.\\
Note 2: Compensation rate = (GGAM IoU - Baseline IoU) / (Good state IoU - Baseline IoU), using points with 0-10\% fragmented neighbor ratio as reference baseline.
...
\caption{Visualization and comparison of segmentation results.}
\label{fig:vis}
\end{figure}
