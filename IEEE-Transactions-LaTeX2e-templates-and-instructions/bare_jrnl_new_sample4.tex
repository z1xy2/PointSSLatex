...
Mamba-based point cloud analysis methods face difficulties in modeling local geometry structure due to the conflict between sequential state propagation and unordered 3D structure. Mamba processes sequences through the following discretized state space equations:
\begin{equation}
x_k = \bar{A}x_{k-1} + \bar{B}u_k, \quad y_k = \bar{C}x_k + \bar{D}u_k
\end{equation}
where $k$ denotes the position index in the serialized sequence, $x_k$ represents the state vector at position $k$, $u_k$ is the input feature vector, $y_k$ is the output feature vector, and $\bar{A}$, $\bar{B}$, $\bar{C}$, $\bar{D}$ are system matrices for state transition, input projection, output projection, and residual connection, respectively.
