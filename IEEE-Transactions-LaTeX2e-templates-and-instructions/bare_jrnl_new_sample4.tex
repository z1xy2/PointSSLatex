...
We further compare ASD-SSM against several common parameterization strategies, as summarized in \Cref{tab:param_comparison}. The Shared Parameter scheme---in which all three scales share a single $\bar{A}$ matrix---treats all scales identically, failing to adapt to their distinct modeling requirements, yielding the lowest performance. The Independent Parameters scheme achieves the highest segmentation performance, but requires fully independent parameters per scale. The Parameter Interpolation approach linearly interpolates parameters between scales; yet, linear interpolation fails to capture non-linear cross-scale relationships, resulting in limited gains. In contrast, ASD-SSM achieves performance within 0.6\% of the Independent Parameters scheme, demonstrating that dynamic content-aware parameter generation is an effective parameterization strategy.
