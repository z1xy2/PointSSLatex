...
\caption{Visualization of geometric structure loss after serialization in wall-column boundary segmentation. The red point represents the center point to be predicted. Green points indicate spatially and sequentially adjacent neighbors whose geometric information can be effectively learned through Mamba's state propagation. Orange points represent fragmented neighbors that are spatially close but sequentially distant, whose geometric information has decayed when reaching the center point, leading to degraded segmentation performance at complex boundaries.}
\label{fig:analysis}
\end{figure}
...
\caption{Overall architecture of the proposed PointSS model. The framework consists of data processing with one-time serialization, GGAM module for global geometric feature perception, and U-shaped encoder-decoder architecture with ASD-SSM modules for hierarchical feature learning. Orange boxes indicate repeated points for uniform window division. The encoder performs downsampling through pooling layers while the decoder uses upsampling. Skip connections link encoder and decoder at each resolution level.}
...
\caption{Structure of the Global Geometry-Aware Mechanism (GGAM). The module operates on dual serialization paths (Z-order and Hilbert encoding) to extract 8-dimensional geometric features including relative coordinates ($\Delta\mathbf{p}_{ij}$), directional relationships ($\alpha_{ij}$), and curvature ($\boldsymbol{\kappa}$). Edge features are aggregated to node features $\mathbf{H}_z$ and $\mathbf{H}_h$. The dual paths undergo self-attention and cross-attention to generate enhanced features, which are adaptively fused through a gating mechanism and geometric consistency constraint to produce final features $\mathbf{H}_{final}$.}
\label{fig:ggam}
\end{figure}
...
\caption{Encoder architecture with hierarchical ASD-SSM modules. The encoder employs progressive downsampling through max pooling layers, with channel dimensions expanding as (32, 64, 128, 256, 512). At each resolution level, multiple ASD-SSM blocks perform deep feature understanding through adaptive scale-decoupled state space modeling. The decoder mirrors this architecture but uses upsampling instead of pooling with channel sizes (64, 64, 128, 256), learning hierarchical information from different encoder stages.}
\label{fig:coder}
\end{figure}
...
\caption{Detailed processing flow of ASD-SSM with $S=3$ scale layers. Window partitioning divides the point cloud into patches with expansion factors $F_s \in \{1, 2, 4\}$ producing sizes $\{P, 2P, 4P\}$ (dashed lines separate patches). Order prompting injects position information at the finest scale while shuffling points at higher scales. Each scale processes features through bidirectional SSMs with adaptive parameters $\overline{A}_s$ generated by a lightweight parameter generator. Scale constraint factors $\alpha_s \in \{0.3, 0.6, 0.9\}$ enable fine scales for rapid response and coarse scales for long-term memory. Multi-scale features are concatenated and mapped to produce final representation $Points' \in \mathbb{R}^{N \times C}$.}
\label{fig:bss}
\end{figure}
...
\caption{Qualitative comparison of semantic segmentation results on S3DIS dataset. The figure shows ground truth annotations alongside predictions from baseline methods and PointSS. Circled regions highlight segmentation differences. PointSS demonstrates superior performance on objects requiring large receptive fields (windows, columns) and complex boundary regions (wall-column junctions), as well as better fine-grained segmentation for small objects (clutter, furniture parts), validating the effectiveness of GGAM's global geometric awareness and ASD-SSM's multi-scale feature fusion.}
\label{fig:vis}
\end{figure}
