...
\author{Xinyuan Zhang$^{1,2,3}$, Xin Wang$^{1,2,3}$
...
\IEEEPARstart{P}{oint} cloud analysis has been widely applied in autonomous driving and augmented reality (AR)/virtual reality (VR) domains. Compared to 2D images, point clouds possess characteristics such as sparsity and disorder, requiring new approaches distinct from 2D image analysis. With the maturation of point cloud reconstruction techniques, large-scale high-density point cloud scenes are gradually becoming prevalent. These large-scale scenes bring challenges to point cloud analysis tasks \cite{Vertex}, requiring more efficient and accurate processing methods, such as improvements in point cloud processing efficiency or feature learning performance. As point cloud density increases, the number of points within the same window also increases, placing higher demands on the model's contextual understanding capabilities.
...
Multi-scale feature fusion plays an important role in point cloud analysis, but existing methods still face challenges \cite{multiscale_survey_2024}: First, scale selection issues—how to automatically determine appropriate scale ranges and scale intervals requires further research; Second, fusion weight learning issues—the importance of different scale features may vary across different regions and tasks, and how to design adaptive weight learning mechanisms is a key problem; Third, computational complexity issues—multi-scale processing often brings additional computational overhead, and optimizing computational efficiency while ensuring performance remains a research focus. Particularly in long-sequence point cloud processing, existing methods struggle to achieve effective information propagation and weight allocation across different scales, limiting model understanding capabilities for complex scenes.
...
\caption{Overall architecture of the proposed PointSS model for point cloud semantic segmentation. The framework consists of three main stages: (1) Data processing with augmentation and one-time point cloud serialization using PTv3's approach; (2) GGAM module for global geometric feature perception through dual serialization and cross-attention mechanisms; (3) U-shaped encoder-decoder architecture with ASD-SSM modules for hierarchical feature learning. The encoder performs downsampling through pooling layers (orange boxes indicate repeated points for uniform window division), while the decoder uses upsampling for feature reconstruction. Skip connections link encoder and decoder at each resolution level. The model outputs final semantic predictions based on maximum predicted probability of labels. Compared to prior methods, this architecture requires only one-time serialization and achieves efficient downsampling through pooling rather than multiple KNN-based serializations.}
...
\caption{Structure of the Global Geometry-Aware Mechanism (GGAM). The module operates on dual serialization paths (Z-order and Hilbert encoding) to compensate for geometric information loss in Mamba-based methods. For each serialization path, windowed neighborhoods are constructed with $O(N)$ complexity to extract 8-dimensional geometric features including relative coordinates ($\Delta\mathbf{p}_{ij}$), directional relationships ($\alpha_{ij}$), and curvature ($\boldsymbol{\kappa}$). Edge features are aggregated to node features $\mathbf{H}_z$ and $\mathbf{H}_h$ through mean pooling. The dual paths undergo self-attention and cross-attention to generate complementary enhanced features $\mathbf{H}''_z$ and $\mathbf{H}''_h$, which are then adaptively fused through a gating mechanism (\cref{eq:adaptive_gate}, \cref{eq:gated_fusion}). Finally, a geometric consistency constraint (\cref{eq:consistency_factor}) ensures the fused features $\mathbf{H}_{final}$ conform to physical geometric laws. This explicit geometric prior injection enables effective global associations between spatially adjacent but sequentially fragmented points.}
\label{fig:ggam}
\end{figure}
...
Given point cloud $\{p_1, p_2, \ldots, p_N\}$ and its serialized point sequence $Q$, i.e., $\{q_1, q_2, \ldots, q_N\}$, we partition it into non-overlapping windows:
\begin{equation}
W_m = \{q_{(m-1) \cdot P + 1}, q_{(m-1) \cdot P + 2}, \ldots, q_{m \cdot P}\}
\end{equation}
where $m = 1, 2, \ldots, \lfloor N/P \rfloor$ is the window index, and $P$ is the window size (Patch Size). During partitioning, the last window may have fewer than $P$ points; in this case, we use a window padding strategy to fill the last window with points from the second-to-last window. Detailed procedures are described in the Window Partitioning section.
...
Based on windowed neighborhoods, we construct edge connection relationships for the point cloud. For points within each window $W_m$, we establish full connections:
\begin{equation}
...
\textbf{Cross-Attention Mechanism Based on Dual Spatial Encoding}: Since we compute the above process in parallel using point clouds serialized with Z-order and Hilbert encoding, each point obtains features $\mathbf{H}_{z}$ and $\mathbf{H}_{h}$ respectively. These two encoding methods produce different spatial neighborhood relationships and node arrangement orders. Z-order encoding, due to its recursive binary partitioning characteristics, has good spatial clustering at local scales and can effectively detect geometric boundaries and local variations. Hilbert curves have better spatial continuity preservation characteristics, better maintaining overall geometric structure coherence during serialization. Following \cite{attention}, we generate $Q$, $K$, $V$ within each encoding sequence for self-attention computation, enabling nodes to attend to neighbor node features under that encoding, with Z-order and Hilbert encoding sequences generating results $A_z$ and $A_h$ respectively. Then we establish cross-attention mechanisms between the two encoding sequences, generating ${A}'_z$ and ${A}'_h$ respectively, enabling feature representations of the same point under two different spatial orderings to mutually enhance. Finally, through feature fusion, we obtain richer and more robust node representations, with specific fusion methods shown in \cref{fig:ggam}. After fusion, we obtain dual serialization-enhanced features $\mathbf{H}''_z$ and $\mathbf{H}''_h$.

\textbf{Adaptive Gated Fusion}: After obtaining enhanced features, we design the final fusion step. To more intelligently fuse the two features, we design an adaptive gating unit. It receives concatenated features and generates a weight for each feature flow (Z-order and Hilbert):
\begin{equation}
...
\cref{fig:coder} shows our encoder's basic architecture, including pooling layers for downsampling and point cloud information aggregation, and multiple repeated ASD-SSM modules for deep feature understanding. We integrate ASD-SSM into the encoder and decoder to achieve high-performance point cloud perception. Unlike the encoder, the decoder uses upsampling to replace Max Pooling. Since the decoder also contains ASD-SSM, it can learn hierarchical information from different encoder stages during upsampling.

\begin{figure}[!t]
\centering
\includegraphics[width=3.5in]{../picture/coder.JPG}
\caption{Encoder architecture with hierarchical ASD-SSM modules. The encoder employs a point-wise feature extraction strategy with progressive downsampling through max pooling layers. At each resolution level, multiple repeated ASD-SSM blocks perform deep feature understanding through adaptive scale-decoupled state space modeling. Channel dimensions expand progressively (32, 64, 128, 256, 512) to capture increasingly abstract semantic representations. The decoder (not shown) mirrors this architecture but uses upsampling instead of pooling and operates with channel sizes (64, 64, 128, 256). Both encoder and decoder integrate ASD-SSM modules, enabling the decoder to learn hierarchical multi-scale information from different encoder stages during feature reconstruction. This U-shaped design with skip connections ensures effective information flow between encoding and decoding paths for dense prediction tasks.}
\label{fig:coder}
\end{figure}
...
\caption{Detailed processing flow of the Adaptive Scale-Decoupled State Space Model (ASD-SSM) with $S=3$ scale layers. The framework consists of three core stages: (1) Window partitioning divides the serialized point cloud into patches of different sizes, with window expansion factors $F_s \in \{1, 2, 4\}$ producing patch sizes $\{P, 2P, 4P\}$ (dashed lines separate patches); (2) Order prompting injects learnable position vectors at the finest scale ($s=1$) while shuffling points within patches at higher scales ($s>1$) to respect point cloud disorder; (3) Multi-scale feature modeling where each scale independently processes features through bidirectional SSMs (forward and backward) with scale-adaptive parameters $\overline{A}_s$ dynamically generated by a lightweight parameter generator (\cref{eq:param_modulation}). The scale constraint factors $\alpha_s \in \{0.3, 0.6, 0.9\}$ enforce differentiated state decay characteristics: fine scales achieve rapid response ($\overline{A}_1 \in [0,0.3]$) while coarse scales maintain long-term memory ($\overline{A}_3 \in [0,0.9]$). Multi-scale features are point-wise concatenated and mapped through MLP and linear layers to produce the final representation $Points' \in \mathbb{R}^{N \times C}$ integrating hierarchical information across all scales.}
\label{fig:bss}
\end{figure}
...
\textbf{Padding}: To ensure the point cloud can be evenly divided, we first pad the point cloud to an integer multiple of Patch Size. For total points $N$ and Patch size $P$, if $N$ is not divisible by $P$, we pad by copying the last several points.

\textbf{Patch Grouping}: After completing padding, we evenly group the point cloud sequence. For the bottom layer ($s=1$), the Patch Size is $P$ with window expansion factor $F_1=1$, resulting in partitioned point cloud $Points_1\in\mathbb{R}^{\lceil N/P \rceil\times P \times C}$. For higher scale layers $s>1$, the Patch Size expands to $P\times F_s$, resulting in point cloud $Points_s\in\mathbb{R}^{\lceil N/(P\times F_s) \rceil\times (P\times F_s) \times C}$ for that layer.

\textbf{Boundary Processing}: In actual operation, the last Patch may have insufficient points. To maintain local continuity, we adopt a sliding window strategy: padding the beginning of the last Patch with end points from the second-to-last Patch. Although this produces a small number of repeated points, it ensures points within the last Patch maintain spatial proximity, avoiding introduction of unrelated padding points.

\subsubsection{Order Prompt}

Since ASD-SSM employs multiple serialization strategies (such as Z-order and Hilbert curves), to help the model understand and utilize different point cloud arrangement rules, we introduce an order prompting mechanism. This mechanism informs the model of the current sequence's arrangement rules through learnable vector "flags," enabling the model to learn order-related patterns.

\textbf{Prompt Vector Design}: We create a set of unique learnable vectors $\{v_1, v_2, ..., v_L\}$ for each serialization method, where $L$ is the number of prompt vectors (set to 6-8 in experiments). Before use, these vectors are mapped through linear layers to the same dimension $C$ as current point cloud features. Then, we insert these prompt vectors at the beginning and end positions of each Patch, forming an enhanced sequence.

\textbf{Hierarchical Strategy}: At the finest scale layer ($s=1$), since point clouds are arranged in serialization order, we add order prompts to strengthen position information. At higher scale layers ($s>1$), we shuffle points within the same Patch in $Points_s$, enabling SSM to respect point cloud disorder for feature learning. Since order is already shuffled, order prompts are no longer needed.
...
Traditional SSM methods use the same state transition parameter $\overline{A}$, input projection parameter $\overline{B}$, and output projection parameter $\overline{C}$ to process features at all scales, ignoring differentiated requirements of different scales for temporal modeling characteristics. The state transition parameter $\overline{A}$ (diagonal elements of the diagonal matrix) controls the decay rate of state vectors: when elements of $\overline{A}$ approach 1, slow decay (long-term memory) is achieved; when approaching 0, fast decay (rapid response) is achieved. For coarse-scale features, we need $\overline{A}$ close to an all-ones vector to maintain long-range dependencies; for fine-scale features, we need $\overline{A}$ to have smaller values to rapidly respond to local detail variations.
...
\end{equation}
where $\|$ denotes concatenation. For selective parameters $\overline{B}_s$ and $\overline{C}_s$, we follow the standard selective mechanism from the Mamba paper \cite{Mamba}, dynamically generating from input content through linear projection, which is not elaborated here.

\textbf{Scale Constraint Mechanism}: To achieve differentiated state decay characteristics, we explicitly control the value domain of state transition matrix $\overline{A}_s$ through scale constraint factor $\alpha_s$. In discrete state space models, state updates follow $\mathbf{h}_k = \overline{A}_s \mathbf{h}_{k-1} + \overline{B}_s \mathbf{u}_k$, where diagonal elements of $\overline{A}_s$ determine state decay rate: element values close to 1 achieve slow decay (long-term memory), close to 0 achieve fast decay (rapid response).

To directly constrain the eigenvalue range of $\overline{A}_s$, we first map the parameter generator's output to the $[0,1]$ interval through the sigmoid function:
\begin{equation}
...
\end{equation}
where $\mathrm{TP}_{c}$ is true positives, $\mathrm{FP}_{c}$ is false positives, and $\mathrm{FN}_{c}$ is false negatives. Taking the average across all categories yields mIoU. It is more robust to class imbalance and more commonly used than Accuracy for 3D semantic segmentation tasks. Therefore, we select mIoU as the metric to evaluate point cloud semantic segmentation tasks on S3DIS.

\textbf{OA}: Overall Accuracy is the number of correctly predicted point cloud samples divided by the total number of all samples. It is simple and intuitive, commonly used as an overall metric for classification tasks, so we select OA as the metric to evaluate point cloud classification tasks on ModelNet40.
...
\textbf{Method Comparison Analysis}: Transformer methods (such as PTv3 and PTv2) perform excellently in point cloud analysis. Through ASD-SSM's scale-decoupled modeling and GGAM's global geometric awareness, PointSS achieves higher accuracy under the same experimental settings (75.2\% vs PTv3's 73.2\%). As an additional advantage of the SSM architecture, PointSS inherits the linear time complexity characteristic of state space models. As shown in \cref{tab:parameterization_comparison}, inference time increases for different parameterization schemes are all insignificant (<7\%), demonstrating good scalability potential.
...
\textbf{Performance Improvement Path of Complete Model}: To clearly demonstrate each module's marginal contribution, we provide a progressive performance improvement path as shown in \cref{tab:progressive_improvement}.
...
\caption{Qualitative comparison of semantic segmentation results on S3DIS dataset. The figure shows ground truth annotations alongside predictions from baseline methods and the proposed PointSS model. Circled regions highlight segmentation differences between methods, particularly for challenging categories. PointSS demonstrates superior performance on objects requiring large receptive fields (windows, columns) and complex boundary regions (wall-column junctions), validating the effectiveness of GGAM's global geometric awareness and ASD-SSM's multi-scale feature fusion. Additionally, PointSS achieves better fine-grained segmentation for small and geometrically complex objects (clutter, furniture parts), proving the model's capability to simultaneously capture global context and preserve local detail information through hierarchical multi-scale architecture and explicit geometric feature learning.}
\label{fig:vis}
\end{figure}
