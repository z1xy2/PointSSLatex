...
Based on these results, we further investigate the internal design of the parameter generator (\Cref{tab:param_gen}). By leveraging patch-level global features alongside scale embeddings, the generator produces patch-specific, scale-conditioned $\bar{A}_s$ values that further enhance performance.

Removing global features from the generator yields a content-invariant $\bar{A}_s$ that varies only across scales—similar in spirit to allocating per-scale fixed parameters—achieving 74.6\%. Incorporating global patch features restores content-awareness, enabling $\bar{A}_s$ to adapt to the local geometric content of each patch and yielding 75.2\%. The 0.6\% gap confirms that content-aware parameter generation, rather than scale differentiation alone, is the primary driver of performance gain.
...
\begin{tabular}{llll}
\toprule
\textbf{Scheme} & \textbf{Global Features} & \textbf{Scale Embedding} & \textbf{mIoU (\%)} \\
\midrule
Baseline (Shared) & - & - & 73.4 \\
Scheme 1 & \checkmark & $\times$ & 74.6 \\
Full (Ours) & \checkmark & \checkmark & 75.2 \\
\bottomrule
\end{tabular}
\end{table}
