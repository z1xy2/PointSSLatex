...
% 前面保留你原本关于 FPN 和近期注意力的描述，然后自然过渡到这段：
Multi-scale feature fusion is fundamental to hierarchical representation learning in point cloud analysis, with PointNet++ \cite{PointNet++} establishing the foundation through hierarchical downsampling and local feature aggregation across scales. Recent SSM-based methods have demonstrated strong global modeling capabilities with linear complexity. However, they struggle with effective multi-scale representations. PointMamba \cite{PointMamba} adopts a non-hierarchical architecture operating at a uniform spatial resolution, limiting its ability to capture fine-grained local details and broad semantic context simultaneously. While PCM \cite{pcm} introduces hierarchical structures through progressive downsampling, it employs spatially invariant parameters across different local regions at each scale, preventing dynamic adaptation to varying local geometric characteristics while incurring memory overhead. This inherently limits their effectiveness in multi-scale feature learning across complex 3D scenes.
