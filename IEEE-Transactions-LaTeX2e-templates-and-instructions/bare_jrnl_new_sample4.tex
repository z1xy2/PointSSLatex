...
As shown in \Cref{tab:progressive_improvement}, the two modules address distinct limitations---GGAM compensates for serialization-induced spatial proximity loss while ASD-SSM enables scale-decoupled state transition modeling---yielding a cumulative gain of 4.9\% mIoU over the baseline.
...
To measure the degree of spatial proximity loss for each point after serialization, we treat it as the center point and search for its 128 spatial neighbors, then compute the proportion of these neighbors that are more than 300 sequence positions away from the center point in the serialized sequence. We term this metric the \textit{proximity loss ratio}. We use it to group points by their degree of spatial proximity loss and compare segmentation performance between the baseline and GGAM (\Cref{tab:ggam_fragmentation}).

Experimental results indicate that approximately 33\% of points suffer from the loss of spatial proximity under Hilbert serialization. As this proportion increases from 0--10\% to 20--30\%, baseline performance drops sharply, confirming the impact of serialization on local geometric representation. GGAM effectively mitigates this degradation. Notably, substantial gains are observed in the high proportion group (20--30\%), significantly exceeding improvements in the low proportion group.



Category-wise analysis (\Cref{tab:ggam_category}) further confirms this trend at the category level. Large planar structures (e.g., floor, wall) experience less serialization-induced loss of spatial proximity and exhibit limited gains. As the proximity loss ratio increases, the compensation effect of GGAM becomes more pronounced: categories with complex boundaries or irregular structures (e.g., window, door, board, clutter) show substantially larger improvements. The beam category yields 0\% IoU for both models, as beam instances are extremely sparse in S3DIS Area 5, resulting in near-zero recall for this class. These findings confirm that GGAM provides more pronounced compensation for serialization-induced proximity loss in structurally complex regions.
