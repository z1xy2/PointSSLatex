...
\begin{abstract}
State Space Models (SSMs) offer linear complexity for point cloud analysis but often suffer from geometric information loss and insufficient multi-scale representation due to the irregularity of 3D data serialization. To address these limitations, this paper proposes PointSS, a geometry-aware multi-scale state space framework. First, we introduce a Global Geometry-Aware Mechanism (GGAM) that mitigates the structural degradation caused by serialization. By incorporating spatial priors and a dual-serialization strategy, GGAM effectively recovers local geometric relationships and enhances boundary consistency. Second, we design an Adaptive Scale-Decoupled State Space Model (ASD-SSM) featuring a dynamic parameter generation mechanism. This approach decouples feature modeling across scales, allowing the model to simultaneously capture fine-grained local details and long-range global dependencies. The proposed method integrates efficiently with standard encoder-decoder architectures without relying on pre-trained models. Extensive experiments on the S3DIS and ModelNet40 datasets demonstrate that PointSS achieves state-of-the-art performance in semantic segmentation and classification tasks, exhibiting superior robustness to serialization perturbations while maintaining computational efficiency.
\end{abstract}

\begin{IEEEkeywords}
Geometry-Aware Feature Learning, Multi-Scale Representation, Point Cloud Serialization, Semantic Segmentation, State Space Models
\end{IEEEkeywords}

\section{Introduction}
\IEEEPARstart{P}{oint} clouds are fundamental for 3D perception but challenge feature extraction due to sparsity and disorder. Early methods like PointNet \cite{pointnet,PointNet++} processed points efficiently but often neglected local geometric relationships. Transformers \cite{pt,ptv2,ptv3} subsequently improved semantic representation via self-attention; however, their quadratic complexity O(N$^2$) creates scalability bottlenecks.

Recently, State Space Models (SSMs) \cite{ssm,Mamba}, exemplified by PointMamba \cite{PointMamba} and PCM \cite{pcm}, have been adopted for their linear complexity. Despite this efficiency, mapping 3D data to 1D sequences introduces ``geometric loss" and limits feature granularity. The root of geometric loss lies in the conflict between Mamba's linear state transition and 3D spatial proximity. Serialization using space-filling curves inevitably creates ``fracture points"---neighbors that are spatially close but sequentially distant. In Mamba's recurrent mechanism, information from these distant points decays before reaching the center point, preventing the effective recovery of local attributes like surface normals and curvature. As illustrated in Fig.~1, this information loss is particularly detrimental at object boundaries (e.g., wall-pillar junctions), where spatially adjacent yet sequentially distant context is crucial for accurate segmentation.

The limitations of existing methods stem from coupling serialization strategies with linear modeling. First, rigid preprocessing makes models sensitive to input permutation, undermining the unordered nature of point clouds. Second, standard linear frameworks struggle to assign adaptive weights based on spatial distance, failing to balance local geometric details with global semantic aggregation.

To address these issues, this paper proposes PointSS, an adaptive scale-decoupled state space framework. The main contributions are:

\begin{enumerate}
\item Explicit geometric priors are injected via windowed neighborhoods and dual-serialization fusion. This compensates for serialization-induced fragmentation and enhances boundary characterization.

\item A dynamic parameter generation mechanism enables fine scales to capture local variations and coarse scales to model long-range dependencies, achieving robust hierarchical representation efficiently.

\item PointSS outperforms existing Mamba-based methods, achieving 75.2\% mIoU on S3DIS and 96.0\% accuracy on ModelNet40, with ablation studies validating each component.
\end{enumerate}

\section{Related Work}

In recent years, the field of point cloud analysis has undergone a significant transition from early methods based on geometric priors to deep learning-driven end-to-end models. Researchers have proposed a series of innovative solutions to address challenges such as the sparsity, disorder, and high-dimensional nature of point cloud data. This section systematically reviews related work across three dimensions: the development of deep learning in point cloud semantic analysis, geometric feature enhancement strategies, and multi-scale feature fusion techniques, while analyzing the core ideas, technical contributions, and limitations of each method.

\subsection{Deep Learning-based Point Cloud Semantic Analysis}

PointNet \cite{pointnet} pioneered handling point cloud disorder via symmetric functions but lacked local detail modeling, a limitation addressed by PointNet++ \cite{PointNet++} through hierarchical sampling. Subsequent works like DGCNN \cite{DGCNN} and FusionNet \cite{FusionNet} enhanced adaptability via dynamic graphs and multi-scale fusion. However, these convolution-based methods remain constrained by fixed receptive fields, limiting effective hierarchical feature construction.

Transformers revolutionized the field by leveraging self-attention for global dependency modeling. Point Transformer \cite{pt} integrated positional encoding for spatial awareness, while PTv3 \cite{ptv3} achieved state-of-the-art performance using simplified serialization. Despite their advantages, the quadratic complexity O(N$^2$) creates bottlenecks for large-scale processing, and their global focus often reduces sensitivity to fine-grained local details.

State Space Models (SSMs) \cite{Mamba} offer a promising alternative with linear complexity. Methods like PointMamba \cite{PointMamba} and PCM \cite{pcm} adapt SSMs by serializing 3D clouds into 1D sequences. However, this adaptation introduces two key challenges: ``geometric fragmentation," where fixed serialization separates spatially adjacent points crucial for local structure (e.g., normals), and single-granularity representation, where linear scanning struggles to balance high-frequency local details with long-range semantic associations.
...
Furthermore, PointSS retains the linear time complexity of SSMs. As shown in Table~IX, the method maintains high computational efficiency with only a marginal increase in inference time, validating its scalability for large-scale point cloud scenarios.
...
Ablation studies were conducted on the S3DIS dataset using a progressive strategy to verify individual module contributions. First, the effectiveness of GGAM in addressing geometric loss was examined (Tables IV--VI), followed by the validation of ASD-SSM design choices (Tables VII--X).

The baseline model utilizes a standard single-scale bidirectional Mamba with shared parameters. As shown in Table~III, introducing GGAM alone yields a 2.2\% mIoU improvement. This confirms that extracting geometric priors through windowed neighborhoods and dual-serialization effectively mitigates serialization-induced information loss, thereby enhancing local structural representation.
...
\begin{table}[!t]
\centering
\caption{Incremental Performance Improvement Path on S3DIS}
\label{tab:progressive_improvement}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Model Configuration} & \textbf{mIoU (\%)} & \textbf{Improvement} \\ \midrule
Pure Mamba Baseline & 70.3 & - \\
Baseline + GGAM & 72.5 & +2.2 \\
Baseline + GGAM + Shared-Parameter ASD-SSM & 73.4 & +0.9 \\
Baseline + GGAM + Full ASD-SSM (PointSS) & \textbf{75.2} & +1.8 \\ \midrule
\textbf{Total Improvement} & - & \textbf{+4.9} \\ \bottomrule
\end{tabular}
\end{table}

\section{Experiments}

\textbf{Implementation Details:} We conducted benchmark tests for point cloud analysis on the indoor dataset S3DIS and the classification dataset ModelNet40. All experiments were performed on an Nvidia A6000 GPU. For network training, we employed the AdamW optimizer and the OneCycleLR learning rate scheduler, with an initial maximum learning rate of 6e-3. The scheduling mechanism included an early-stage warm-up followed by a cosine annealing cycle. The entire training process spanned 3000 epochs with a batch size of 6, utilizing Automatic Mixed Precision (AMP) to enhance training efficiency.
...
\textbf{ModelNet40:} This dataset is utilized for 3D object classification, comprising 12,311 CAD models from 40 categories. The data is partitioned into 8,156 training samples and 4,155 testing samples.

\subsection{Evaluation Metrics}

\textbf{mIoU:} Mean Intersection over Union (mIoU) is a widely used evaluation metric in tasks such as semantic segmentation to measure the degree of overlap between predicted results and ground truth labels. For each category $c$, the Intersection over Union (IoU) is defined as:
\begin{equation}
\text{IoU}_c = \frac{\text{TP}_c}{\text{TP}_c + \text{FP}_c + \text{FN}_c}
\end{equation}
where TP$_c$ denotes the number of points correctly predicted as category $c$ (True Positives), FP$_c$ represents the number of points incorrectly predicted as category $c$ (False Positives), and FN$_c$ represents the number of points that truly belong to category $c$ but were not correctly predicted (False Negatives). The mIoU is obtained by taking the arithmetic mean of the IoU values across all semantic categories. Since mIoU provides a more comprehensive reflection of the model's segmentation performance across various classes---especially in cases of imbalanced class distributions---it is more robust than Overall Accuracy. Therefore, this paper adopts mIoU as the primary metric for evaluating point cloud semantic segmentation performance on the S3DIS dataset.
...
\begin{tabular}{llll}
\toprule
\textbf{Source} & \textbf{Model} & \textbf{Architecture} & \textbf{mIoU} \\
\midrule
NeurIPS22 & Point Transformer V2 \cite{ptv2} & Transformer & 72.6 \\
CVPR23 & PointMetaBase \cite{pmb} & Transformer & 72.0 \\
ICCV23 & SuperpointTransformer \cite{spt} & Transformer & 68.1 \\
...
\begin{tabular}{llll}
\toprule
\textbf{Source} & \textbf{Model} & \textbf{Architecture} & \textbf{OA} \\
\midrule
NeurIPS22 & Point Transformer V2 \cite{ptv2} & Transformer & 94.2 \\
AAAI24 & Point-FEMAE \cite{FEMAE} & Transformer & 94.5 \\
LNCS24 & Point2Vec \cite{Point2Vec} & Transformer & 94.8 \\
