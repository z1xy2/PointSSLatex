...
Experimental results indicate that both patch-level global features and scale embeddings independently improve model performance, with global features providing a slightly higher gain than scale embeddings alone. This suggests that patch-level contextual information provides a critical reference for state parameter generation, enabling the model to adaptively adjust $\bar{A}_s$ across different spatial locations. When both sources are integrated concurrently, the combined gain is substantially higher than from either source alone, demonstrating a synergistic effect in the parameter generation process. Further analysis of different modulation strengths ($\lambda$) reveals that optimal performance is achieved at $\lambda = 0.1$. Insufficient modulation strength limits the model's adaptive capacity, while excessive strength may introduce destabilizing perturbations to the state transition dynamics.
