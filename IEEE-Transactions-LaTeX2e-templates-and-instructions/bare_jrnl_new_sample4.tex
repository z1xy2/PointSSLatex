...
The overall architecture of the PointSS model is illustrated in \cref{fig:architecture}. Following data augmentation, we serialize point clouds into 1D sequences\cite{ptv3}. Following data processing, we employ the Global Geometry-Aware Mechanism (GGAM) to capture global geometric priors.
The feature learning component utilizes a PointNet-based U-shaped encoder-decoder architecture. Note that some points may appear in multiple patches (these points are highlighted in orange) to ensure each patch contains an equal number of points. Finally, the fused features are fed into a prediction head. The ultimate result, whether for individual points or the entire point cloud, is determined by selecting the category with the highest probability. Compared to PCM, this scheme only requires serialization at the beginning of training. It performs down-sampling directly via pooling on the serialized point cloud,rather than performing re-serialization at each downsampling stage as in PCM. This approach significantly reduces computational complexity.
