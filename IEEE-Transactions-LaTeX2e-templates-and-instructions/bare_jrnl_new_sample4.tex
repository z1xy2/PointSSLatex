...
The baseline model utilizes a standard single-scale bidirectional Mamba. As shown in \Cref{tab:progressive_improvement}, introducing GGAM alone yields a notable mIoU improvement. This confirms that extracting geometric priors through windowed neighborhoods and dual-serialization effectively mitigates the serialization-induced loss of spatial proximity, thereby enhancing local structural representation.

Building on this, employing ASD-SSM with scale-aware parameter generation boosts performance to 75.2\%, providing a further marked gain. This demonstrates that scale-decoupled parameterization is crucial for simultaneously capturing local details and modeling global semantics.

GGAM and ASD-SSM are complementary by design, together yielding a substantial overall improvement.
...
\begin{table*}[!t]
	\centering
	\caption{Point Distribution Across Different Proximity Loss Levels and the Compensation Effect of GGAM}
	\label{tab:ggam_fragmentation}
	\begin{tabular}{llllll}
		\toprule
		\textbf{Displacement} & \textbf{Point} & \textbf{Point} & \textbf{Baseline} & \textbf{GGAM} & \textbf{Absolute} \\
	...
	\end{tabular}
	\par\vspace{3pt}
	\footnotesize Note: The displacement ratio is defined as the proportion of a point's K-nearest spatial neighbors whose position in the serialized sequence is more than 300 apart from the center point.
\end{table*}
\subsubsection{Ablation Study of GGAM}

We verify the effectiveness of GGAM through three experiments on the S3DIS dataset.

To measure the degree of spatial proximity loss for each point after serialization, we treat it as the center point and search for its 128 spatial neighbors in 3D space, then compute the proportion of these neighbors whose position in the serialized sequence is more than 300 apart from the center point. We use this proportion to group points by their degree of spatial proximity loss and compare segmentation performance between the baseline and GGAM (\Cref{tab:ggam_fragmentation}).

Experimental results indicate that approximately 33\% of points suffer from the loss of spatial proximity under Hilbert serialization. As this proportion increases from 0--10\% to 20--30\%, baseline performance drops sharply, confirming the detrimental impact of serialization on local geometric representation. GGAM effectively mitigates this degradation, with larger gains at higher levels of spatial proximity loss. Notably, substantial gains are observed in the high proportion group (20--30\%), significantly exceeding improvements in the low proportion group.



Category-wise analysis (\Cref{tab:ggam_category}) shows that categories with more severe spatial proximity loss also benefit more from GGAM. Large planar structures with high continuity (e.g., wall, floor) show limited gains . In contrast, categories featuring complex boundaries and slender structures (e.g., column, clutter) exhibit a higher proportion of neighbors far apart in the serialized sequence and achieve substantial improvements. These findings demonstrate that GGAM is particularly effective at mitigating the loss of spatial proximity in regions with sharp transitions and dense boundaries.
...
\begin{tabular}{llllll}
\toprule
\textbf{Category} & \textbf{Avg. Displacement Ratio} & \textbf{Baseline IoU (\%)} & \textbf{GGAM IoU (\%)} & \textbf{Absolute Gain (\%)} & \textbf{Geometric Characteristics} \\
\midrule
...
Table & 11\% & 73.5 & 75.2 & +1.7 & Rectangular surface with legs \\
Bookshelf & 13\% & 70.2 & 71.7 & +1.5 & Vertical structure \\
Chair & 15\% & 68.9 & 71.2 & +2.3 & Complex structure \\
Sofa & 15\% & 70.6 & 73.4 & +2.8 & Curved surface with backrest \\
Door & 17\% & 65.2 & 67.5 & +2.3 & Rectangular boundaries \\
Column & 18\% & 54.3 & 58.1 & +3.8 & Slender vertical structure \\
Window & 19\% & 61.8 & 65.8 & +4.0 & Complex boundaries \\
Clutter & 22\% & 45.7 & 49.2 & +3.5 & Highly complex \\
\midrule
Mean & 13.5\% & 70.3 & 72.5 & +2.2 & - \\
\bottomrule
\end{tabular}
\par\vspace{3pt}
\footnotesize Note: The Avg. Displacement Ratio is defined as the mean displacement ratio across all points belonging to a specific semantic category.
\end{table*}

We further ablate the remaining components of GGAM (\Cref{tab:ggam_components}) to verify their individual contributions and synergistic effects. 
Introducing dual-serialization alone yields limited improvement or even slight fluctuation, as different serialization methods may introduce biases without an effective fusion strategy. Adding cross-sequence attention further yields a marked improvement, demonstrating that cross-sequence interaction plays a role in aligning complementary geometric information across different serialization patterns. Incorporating the gated fusion mechanism and geometric consistency constraint further refines the fused representation, as the gated mechanism adaptively balances contributions from different serialization branches while the consistency constraint suppresses unreasonable geometric combinations caused by window-approximated neighborhoods.

The overall improvement demonstrates that these components work synergistically to mitigate the serialization-induced loss of spatial proximity.
...
\end{tabular}
\par\vspace{3pt}
\footnotesize Notes: DS: Dual-Serialization; CSA: Cross-Sequence Attention; GFM: Gated Fusion Mechanism; $L_{geo}$: Geometric Consistency Constraint (Loss). All configurations retain geometric feature extraction as it is a prerequisite of GGAM.
\end{table}
