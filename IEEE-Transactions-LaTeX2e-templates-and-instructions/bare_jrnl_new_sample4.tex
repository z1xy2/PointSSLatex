...
Notation: $s \in \{1, 2, \ldots, S\}$ denotes the scale index, where $s=1$ represents the finest scale (base patch size $P$) and $s=S$ the coarsest (patch size $P \times F_S$). $N$ denotes the number of points and $C$ the feature dimension.

As illustrated in \cref{fig:bss}, the pipeline comprises three stages. First, in \textbf{Window Partitioning}, the serialized sequence is uniformly partitioned into non-overlapping patches; if the last patch is incomplete, it is padded following the strategy of PTv3~\cite{ptv3}. At scale $s$, the patch size is $P \times F_s$ (with $F_1 = 1$ for the finest scale); for a three-scale configuration ($S=3$), the expansion factors are $F_2=2$ and $F_3=4$. Second, following \cite{pcm}, \textbf{Order Prompting} inserts learnable prompt vectors at patch boundaries at the finest scale to inform Mamba of the sequence's arrangement rules; at coarser scales, points within each patch are randomly shuffled and no prompts are inserted. Third, \textbf{Multi-scale Feature Modeling}—the core contribution of ASD-SSM—processes each scale through a bidirectional Mamba whose state transition parameters $\bar{A}$ are dynamically adapted to the local geometric content of each patch by a scale-aware parameter generator. After feature extraction across all scales, the representations are concatenated point-wise and projected back to $C$ dimensions via a linear layer and an MLP, yielding the final point cloud representation $\text{Points}' \in \mathbb{R}^{N \times C}$.
...
While existing SSM-based methods leverage input-dependent $\bar{B}$ and $\bar{C}$ via the selective mechanism, the state transition matrix $\bar{A}$ remains input-invariant—applied uniformly across both spatial locations and scales—overlooking the distinct sequence modeling requirements at different granularities. The diagonal elements of $\bar{A}$ control the state decay rate: values close to 1 yield slow decay (long-term memory), while values close to 0 yield fast decay (rapid response to local variations). Consequently, coarse-scale features require $\bar{A}$ values close to 1 to capture long-range dependencies, whereas fine-scale features require smaller values to respond quickly to local geometric detail.

Scale-aware Parameter Generator: ASD-SSM adaptively generates scale-specific state transition parameters $\bar{A}_s$ for different scales. For a given scale $s$, the point cloud features are partitioned into patches, and the parameter generation process is conducted independently for each patch, producing a patch-specific $\bar{A}_s$ conditioned on the content features of that patch.

Specifically, we first apply global average pooling over all points within each patch to obtain the patch-level feature $\mathbf{f}_{\text{global}}^{(s)}$, which captures the overall geometric characteristics of the patch (e.g., local structural complexity and density distribution). Simultaneously, each scale $s$ is associated with a learnable embedding vector $\mathbf{e}_s \in \mathbb{R}^{C/4}$, retrieved from a trainable embedding table indexed by $s$, which serves as a scale identifier encoding scale-specific context. For a single patch, the parameter generation process is as follows:
\begin{align}
\tilde{\mathbf{f}}_{\text{global}}^{(s)} &= \text{MLP}_{\text{global}}(\mathbf{f}_{\text{global}}^{(s)}) \in \mathbb{R}^{C/4} \\
\mathbf{c}_s &= \text{MLP}([\tilde{\mathbf{f}}_{\text{global}}^{(s)} \| \mathbf{e}_s]) \in \mathbb{R}^{C/2}
\end{align}
...
\end{equation}
where $\sigma(\cdot)$ is the sigmoid activation function, and $\lambda$ is a scaling factor (set to 0.1 in experiments) that controls the amplitude of the MLP output. Then, the scale constraint factor $\alpha_s$ serves as an upper bound to directly control the value range of $\bar{A}_s$:
\begin{equation}
\bar{A}_s = \alpha_s \cdot \hat{A}_s^{\text{raw}}
\end{equation}

By setting $\alpha_s$ to increase monotonically from the finest to the coarsest scale (from $\alpha_1 = 0.3$ to $\alpha_S = 0.9$), we impose an explicit constraint: the fine-scale $\bar{A}_1 \in [0, 0.3]$ enables rapid response to local variations, while the coarse-scale $\bar{A}_S \in [0, 0.9]$ maintains long-term memory to capture global context. It should be noted that, although the patch index is omitted in the notation for simplicity, each patch generates a patch-specific parameter $\bar{A}_s^{(m)}$ conditioned on its own content features, where $m$ denotes the patch index. Specifically, $\bar{A}_s^{(m)} = \alpha_s \cdot \hat{A}_s^{\text{raw},(m)}$, where $\hat{A}_s^{\text{raw},(m)} = \sigma(\lambda \cdot \text{MLP}([\tilde{\mathbf{f}}_{\text{global}}^{(s,m)} \| \mathbf{e}_s]))$.
...
\end{align}
where $\mathbf{h}_k^{(s,m)}$ is the hidden state vector for scale $s$ and patch $m$, $\mathbf{u}_k^{(s,m)}$ is the input feature of the $k$-th point within the patch, and $\bar{D}$ denotes the skip connection parameter. For the $m$-th patch, the initial state is set as $\mathbf{h}_0^{(s,m)} = \mathbf{h}_{\text{last}}^{(s,m-1)}$ (initialized as a zero vector for the first patch). This cross-patch state propagation ensures that state transitions at patch boundaries remain coherent with those within the patches.

Bidirectional Feature Learning and Fusion: A bidirectional feature learning strategy is employed within each scale. We construct a forward SSM and a backward SSM for each scale $s$, processing the serialized point sequence in forward and reverse orders, respectively, so that each point aggregates context from both directions. The forward and backward SSMs share the same set of scale-specific state transition parameters $\bar{A}_s$. The resulting bidirectional features are then fused to produce the scale-$s$ output.

Through the aforementioned mechanisms, ASD-SSM achieves scale-decoupled state-space modeling: at the finest scale ($s = 1$), $\alpha_1 = 0.3$ confines $\bar{A}_1 \in [0, 0.3]$, enabling rapid state decay for local detail capture; at the coarsest scale ($s = S$), $\alpha_S = 0.9$ allows $\bar{A}_S \in [0, 0.9]$, enabling slow state decay for long-range global context. Upon completion of feature extraction across all scales, the features $\text{Points}_{1:S}$ are concatenated channel-wise at each point position. The concatenated representation is then mapped back to the original dimension $C$ through an MLP followed by a linear projection, yielding the multi-scale fused point cloud representation $\text{Points}' \in \mathbb{R}^{N \times C}$.
