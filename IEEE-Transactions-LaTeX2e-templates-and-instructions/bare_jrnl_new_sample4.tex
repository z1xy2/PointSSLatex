...
semantic segmentation and classification tasks while achieving faster inference and superior scalability compared to Transformer-based counterparts.
\end{abstract}
...
analysis. The main contributions of this work are as follows:
  ...
  \begin{enumerate}
	...
	art performance, reaching 75.2\% mIoU on S3DIS and 94.6\% accuracy on
...
\end{equation}
where $m = 1, 2, \ldots, \lfloor N/64 \rfloor$ is the window index, and the window size is fixed at 64. A smaller window confines the neighborhood to spatially proximate points, which is essential for accurate estimation of local geometric properties such as surface normals and curvature, while 64 points provides a sufficient sample for stable PCA-based estimation with modest computational overhead. If the last window has fewer than $64$ points, it is padded following the strategy of PTv3~\cite{ptv3}.
...
CVPR24 & Point Transformer V3\cite{ptv3} & Transformer & 73.4 \\
...
\cref{tab:ModelNet} summarizes the classification performance on ModelNet40. PointSS achieves an Overall Accuracy (OA) of 94.6\% without pre-training, surpassing all SSM-based counterparts including Mamba3D (94.1\%) and PointMamba (93.6\%), while remaining competitive with Transformer-based methods. Note that on this near-saturated benchmark, most methods cluster within a narrow accuracy range, and marginal differences are within experimental variance.
...
& PointSS & SSM & \textbf{94.6} \\ \bottomrule
\end{tabular}
\end{table}
...
This paper presents PointSS, a geometry-aware multi-scale framework for point cloud analysis built upon State Space Models. GGAM addresses the loss of spatial proximity caused by serialization through windowed neighborhood modeling and dual-serialization fusion, providing reliable geometric guidance throughout the network. ASD-SSM addresses the input-invariant limitation by adapting $\bar{A}$ to local geometric characteristics at each scale, enabling effective hierarchical representations. Experiments on semantic segmentation and 3D object classification benchmarks demonstrate state-of-the-art performance, with particular improvements in boundary regions and fine-grained structures, suggesting that combining explicit geometric priors with adaptive sequence modeling is a promising direction for 3D point cloud analysis.
