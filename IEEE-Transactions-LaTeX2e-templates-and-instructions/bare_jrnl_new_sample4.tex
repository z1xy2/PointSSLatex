...
based methods like PTv2 \cite{ptv2} and PTv3 \cite{ptv3} adopt U-Net
architectures with hierarchical attention mechanisms. These approaches
demonstrate the importance of explicit multi-scale modeling for balancing
local geometric details and global semantic understanding.
...
a plain, non-hierarchical Mamba backbone, relying on space-filling curves
(Z-order and Hilbert) for the serialization of point clouds. This design prioritizes
...
coordinate orders and employs separate Mamba parameters for each downsampled
scale. While this multi-scale design improves upon PointMamba's single-scale
limitation, PCM's parameters remain shared across all spatial locations within
each scale. Consequently, the model applies uniform state transitions regardless
of local point distributions—whether processing densely sampled flat regions or
sparsely distributed boundary areas. Additionally, maintaining independent
parameter sets for each scale and each serialization variant significantly
increases memory footprint and parameter count.

These design choices reveal a fundamental trade-off in SSM-based point cloud
methods: simplicity and parameter efficiency (PointMamba) versus multi-scale
capability but with spatial invariance and memory overhead (PCM). Neither
approach achieves input-dependent, scale-adaptive parameterization that can
dynamically adjust to local geometric characteristics while maintaining
computational efficiency across hierarchical scales.
