...
\end{equation}
The resulting feature $\mathbf{H}_i \in \mathbb{R}^C$ encodes the local geometric context of $q_i$. Since graph construction runs in parallel on Z-order and Hilbert serialization methods, each point obtains two independent geometric feature vectors, $\mathbf{H}_z$ and $\mathbf{H}_h$, which are passed to the second part of GGAM.
...
Each stream is first normalized before computing attention. Let $\hat{\mathbf{H}}_z = \text{LayerNorm}(\mathbf{H}_z)$ and $\hat{\mathbf{H}}_h = \text{LayerNorm}(\mathbf{H}_h)$. Self-attention is then applied within each normalized stream:
\begin{equation}
\begin{aligned}
A_z = \text{Attention}(\hat{\mathbf{H}}_z, \hat{\mathbf{H}}_z, \hat{\mathbf{H}}_z), \\
A_h = \text{Attention}(\hat{\mathbf{H}}_h, \hat{\mathbf{H}}_h, \hat{\mathbf{H}}_h)
\end{aligned}
\end{equation}
with residual connections:
\begin{equation}
\mathbf{H}'_z = \hat{\mathbf{H}}_z + A_z, \quad \mathbf{H}'_h = \hat{\mathbf{H}}_h + A_h
\end{equation}
...
\begin{equation}
\begin{aligned}
A'_z = \text{Attention}(\mathbf{H}'_z, \mathbf{H}'_h, \mathbf{H}'_h), \\
A'_h = \text{Attention}(\mathbf{H}'_h, \mathbf{H}'_z, \mathbf{H}'_z)
\end{aligned}
\end{equation}
The final output is obtained via residual addition:
\begin{equation}
\begin{aligned}
\mathbf{H}''_z = \mathbf{H}'_z + A'_z, \quad \mathbf{H}''_h = \mathbf{H}'_h + A'_h
\end{aligned}
\end{equation}
