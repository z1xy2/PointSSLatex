...
To fairly evaluate the independent contributions of our proposed architectural innovations (GGAM and ASD-SSM), we adopted a train-from-scratch setting, consistent with current Mamba-based methods \cite{pcm, PointMamba}. This choice ensures that performance gains are solely attributable to architectural improvements, eliminating confounding factors arising from diverse pre-training strategies.

\subsection{Datasets}

\textbf{S3DIS.} The S3DIS dataset serves as a standard benchmark for indoor semantic segmentation. It contains 272 rooms across six distinct areas, covering 13 semantic categories (e.g., walls, furniture). Each point includes 3D coordinates and RGB information. Following standard evaluation protocols, we report results on Area 5.
