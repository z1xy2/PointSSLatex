...
前两章分别提出了GGAM和ASD-SSM两个核心创新点，从几何特征增强和多尺度特征学习两个角度提升了PointSS在固定序列化策略下的性能。GGAM通过显式注入几何先验补偿了Mamba的几何学习不足（第三章），ASD-SSM通过自适应尺度解耦实现了层次化的多尺度特征学习（第四章）。然而，无论是GGAM还是ASD-SSM，都基于PTv3提供的固定序列化顺序（如Z-order或Hilbert曲线）进行特征学习。这种固定策略存在一个根本性局限：同一种扫描顺序被应用于所有场景和所有区域，无法适应不同场景的几何特性和语义结构。

具体而言，固定序列化策略面临三个关键问题：\textbf{场景无关性}——无法根据场景的几何特性调整序列化策略；\textbf{重要性忽视}——无法优先处理几何上或语义上更重要的区域（如物体边界、关键结构）；\textbf{局部性受限}——不同几何结构（如平面、曲面、边缘）适合不同的序列化方式，固定策略无法兼顾。受自然语言处理中混合专家模型（Mixture of Experts）和动态神经路由的启发，我们提出一个根本性问题：能否让模型根据局部几何特性，\textbf{动态选择}最适合的序列化路径？

本章提出\textbf{多路径自适应序列化网络}（Multi-path Adaptive Serialization Network, MASA）作为PointSS的第三个核心创新点，从序列化策略的角度进一步提升模型性能。与GGAM和ASD-SSM在"给定序列化下如何更好地学习"不同，MASA解决的是"如何选择更好的序列化"的问题。MASA通过几何条件化的动态路由机制，根据局部几何特性自适应选择或组合序列化路径，将Mamba的选择机制从\textit{特征选择}（selecting features）扩展到\textit{路径选择}（selecting paths）。三个创新点协同作用，共同构成PointSS的完整技术体系。

本章结构安排如下：5.1节阐述MASA的问题建模与动机，5.2节详细介绍方法设计（包括多路径序列化策略、几何感知的重要性场预测、动态路由网络等），5.3节通过广泛的实验和可视化分析验证MASA的有效性，5.4节对本章进行总结并展望未来方向。
